...
const { pool, query, initDB, seedCards, seedAdmin } = require('./db');
...
function advanceFainted(battle) {
  if (battle.playerCards[battle.playerActive]?.current_hp <= 0) {
    ...
    if (next !== -1) {
      battle.playerActive = next;
      battle.log.push(`Your ${battle.playerCards[next].name} steps forward!`);
      battle.playerRevengeActive = true;
      battle.playerRevengeTurns = 2;
      battle.log.push(`⚡ Revenge! ${battle.playerCards[next].name} gains +20% ATK for 2 turns!`);
    }
  }
  if (battle.aiCards[battle.aiActive]?.current_hp <= 0) {
    const next = battle.aiCards.findIndex((c,i) => i !== battle.aiActive && c.current_hp > 0);
    if (next !== -1) {
      battle.aiActive = next;
      battle.log.push(`Foe sends out ${battle.aiCards[next].name}!`);
    }
  }
}
...
function runAiTurn(battle) {
  ...
  if ((aiActive.orbs || 0) >= cost) {
    aiActive.orbs -= cost;
    let dmg = applyWeather(calcDamage(aiActive, pa), aiActive.type, battle.weather);
    ...
    let dmg = applyWeather(Math.floor(rawDmg * speedBonus * coachDefMult), aiActive.type, battle.weather);
    if (battle.playerGuarded) { dmg = Math.floor(dmg * 0.5); battle.log.push(`🛡️ Your guard absorbed half the damage!`); }
    pa.current_hp = Math.max(0, pa.current_hp - dmg);
...
function rollWeather() {
  const r = Math.random();
  if (r < 0.22) return 'storm';
  if (r < 0.44) return 'drought';
  if (r < 0.66) return 'fog';
  return null;
}
function applyWeather(dmg, attackerType, weather) {
  if (!weather) return dmg;
  if (weather === 'storm'   && attackerType === 'Thunder') return Math.floor(dmg * 1.15);
  if (weather === 'drought' && attackerType === 'Fire')    return Math.floor(dmg * 1.15);
  if (weather === 'drought' && attackerType === 'Water')   return Math.floor(dmg * 0.90);
  if (weather === 'fog')    return Math.floor(dmg * 0.90);
  return dmg;
}

// ─── DECK ARCHETYPE ───────────────────────────────────────────────
function detectArchetype(cards) {
  if (!cards || !cards.length) return 'Balanced';
  const types = [...new Set(cards.map(c => c.type))];
  if (types.length === 1) return 'Mono-' + types[0];
  const avgAtk = cards.reduce((s,c) => s + (c.atk||0), 0) / cards.length;
  const avgHp  = cards.reduce((s,c) => s + (c.hp||0),  0) / cards.length;
  const avgDef = cards.reduce((s,c) => s + (c.def||0),  0) / cards.length;
  if (avgAtk > 160 && avgHp < 300) return 'Aggro';
  if (avgDef > 130 && avgHp > 350) return 'Control';
  return 'Balanced';
}

// ─── ACHIEVEMENTS ─────────────────────────────────────────────────
async function checkAchievements(userId, triggerType, currentValue) {
  try {
    const eligible = await query(
      `SELECT a.* FROM achievements a
       WHERE a.trigger_type=$1 AND a.trigger_value <= $2
         AND NOT EXISTS (SELECT 1 FROM user_achievements ua WHERE ua.user_id=$3 AND ua.achievement_id=a.id)`,
      [triggerType, currentValue, userId]
    );
    if (!eligible.rows.length) return [];
    const unlocked = [];
    for (const ach of eligible.rows) {
      await query('INSERT INTO user_achievements (user_id, achievement_id) VALUES ($1,$2) ON CONFLICT DO NOTHING', [userId, ach.id]);
      if (ach.reward_coins > 0) await query('UPDATE users SET coins=coins+$1 WHERE id=$2', [ach.reward_coins, userId]);
      unlocked.push(ach);
    }
    return unlocked;
...
function getPvpStateForUser(battle, userId) {
  ...
  return {
    ...
    playerRevengeActive:  isP1 ? battle.p1RevengeActive : battle.p2RevengeActive,
    playerRevengeTurns:   isP1 ? battle.p1RevengeTurns  : battle.p2RevengeTurns,
  };
}
...
async function finishPvpBattle(battle) {
  try {
    ...
    // Achievement checks for PvP winner
    const winnerId2 = p1Won ? battle.player1Id : battle.player2Id;
    const wRank = await query('SELECT rating FROM ranked_stats WHERE user_id=$1', [winnerId2]);
    const newRating = wRank.rows[0]?.rating || 1000;
    const pvpWins = await query('SELECT wins FROM ranked_stats WHERE user_id=$1', [winnerId2]);
    const [achPvp, achRank] = await Promise.all([
      checkAchievements(winnerId2, 'win_pvp', pvpWins.rows[0]?.wins || 0),
      checkAchievements(winnerId2, 'reach_rank', newRating),
    ]);
    if (!battle.ratingResult) battle.ratingResult = {};
    battle.ratingResult.newAchievements = [...(achPvp||[]), ...(achRank||[])];
    ...
    for (let j = i + 1; j < queue.length; j++) {
      if (queue[i].ranked === queue[j].ranked) {
        ...
        const battle = {
          ...
          battleChat: [],
          weather: rollWeather(),
          p1RevengeActive: false, p1RevengeTurns: 0,
          p2RevengeActive: false, p2RevengeTurns: 0,
        };
        const pvpWMsgs = { storm: '⛈️ A storm brews — Thunder cards deal extra damage!', drought: '☀️ A drought scorches the field — Fire is empowered, Water is weakened!', fog: '🌫️ Thick fog rolls in — all attacks are weakened!' };
        if (battle.weather && pvpWMsgs[battle.weather]) battle.log.push(pvpWMsgs[battle.weather]);
...
app.get('/api/achievements', auth, async (req, res) => {
  try {
    const [all, unlocked] = await Promise.all([
      query('SELECT * FROM achievements ORDER BY category, trigger_value'),
      query('SELECT achievement_id, unlocked_at FROM user_achievements WHERE user_id=$1', [req.user.id]),
    ]);
    const unlockedIds = unlocked.rows.map(r => r.achievement_id);
    res.json({ all: all.rows, unlocked: unlockedIds, unlockedAt: Object.fromEntries(unlocked.rows.map(r => [r.achievement_id, r.unlocked_at])) });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// ─── CARD LEVELING ────────────────────────────────────────────────
app.get('/api/cards/levels', auth, async (req, res) => {
  try {
    const r = await query('SELECT card_id, level, xp FROM user_card_levels WHERE user_id=$1', [req.user.id]);
    const levels = {};
    for (const row of r.rows) levels[row.card_id] = { level: row.level, xp: row.xp };
    res.json(levels);
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/cards/:id/level-up', auth, async (req, res) => {
  try {
    const cardId = parseInt(req.params.id);
    const owned = await query('SELECT quantity FROM user_cards WHERE user_id=$1 AND card_id=$2', [req.user.id, cardId]);
    if (!owned.rows.length || owned.rows[0].quantity < 2)
      return res.status(400).json({ error: 'Need at least 2 copies to level up' });
    const lvlRes = await query('SELECT level, xp FROM user_card_levels WHERE user_id=$1 AND card_id=$2', [req.user.id, cardId]);
    const current = lvlRes.rows[0] || { level: 1, xp: 0 };
    if (current.level >= 5) return res.status(400).json({ error: 'Card is already max level' });
    const xpNeeded = current.level;
    const newXp = current.xp + 1;
    const didLevel = newXp >= xpNeeded;
    const newLevel = didLevel ? current.level + 1 : current.level;
    const leftoverXp = didLevel ? newXp - xpNeeded : newXp;
    await query('UPDATE user_cards SET quantity=quantity-1 WHERE user_id=$1 AND card_id=$2', [req.user.id, cardId]);
    await query(
      'INSERT INTO user_card_levels (user_id,card_id,level,xp) VALUES ($1,$2,$3,$4) ON CONFLICT (user_id,card_id) DO UPDATE SET level=$3, xp=$4',
      [req.user.id, cardId, newLevel, leftoverXp]
    );
    const card = await query('SELECT name FROM cards WHERE id=$1', [cardId]);
    res.json({ level: newLevel, xp: leftoverXp, xpNeeded: newLevel < 5 ? newLevel : 0, leveled: didLevel, cardName: card.rows[0]?.name });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// ─── CARD GIFTING ─────────────────────────────────────────────────
app.post('/api/cards/:id/gift', auth, async (req, res) => {
  try {
    const cardId = parseInt(req.params.id);
    const { recipientUsername } = req.body;
    if (!recipientUsername) return res.status(400).json({ error: 'recipientUsername required' });
    const recipient = await query('SELECT id FROM users WHERE LOWER(username)=LOWER($1)', [recipientUsername]);
    if (!recipient.rows.length) return res.status(404).json({ error: 'User not found' });
    const recipientId = recipient.rows[0].id;
    if (recipientId === req.user.id) return res.status(400).json({ error: "Can't gift to yourself" });
    const friends = await query(
      `SELECT 1 FROM friends WHERE status='accepted' AND ((user_id=$1 AND friend_id=$2) OR (user_id=$2 AND friend_id=$1))`,
      [req.user.id, recipientId]
    );
    if (!friends.rows.length) return res.status(403).json({ error: 'Can only gift cards to friends' });
    const recent = await query(
      `SELECT COUNT(*) FROM card_gifts WHERE sender_id=$1 AND recipient_id=$2 AND gifted_at > NOW() - INTERVAL '24 hours'`,
      [req.user.id, recipientId]
    );
    if (parseInt(recent.rows[0].count) >= 1) return res.status(429).json({ error: 'Already gifted a card to this player in the last 24 hours' });
    const owned = await query('SELECT quantity FROM user_cards WHERE user_id=$1 AND card_id=$2', [req.user.id, cardId]);
    if (!owned.rows.length || owned.rows[0].quantity < 1) return res.status(403).json({ error: 'You do not own this card' });
    if (owned.rows[0].quantity === 1) {
      await query('DELETE FROM user_cards WHERE user_id=$1 AND card_id=$2', [req.user.id, cardId]);
    } else {
      await query('UPDATE user_cards SET quantity=quantity-1 WHERE user_id=$1 AND card_id=$2', [req.user.id, cardId]);
    }
    await query('INSERT INTO user_cards (user_id,card_id) VALUES ($1,$2) ON CONFLICT (user_id,card_id) DO UPDATE SET quantity=user_cards.quantity+1', [recipientId, cardId]);
    await query('INSERT INTO card_gifts (sender_id,recipient_id,card_id) VALUES ($1,$2,$3)', [req.user.id, recipientId, cardId]);
    await query('INSERT INTO notifications (user_id,type,message) VALUES ($1,$2,$3)', [recipientId, 'gift', `${req.user.username} gifted you a card!`]).catch(()=>{});
    const achList = await checkAchievements(req.user.id, 'gift_card', parseInt(recent.rows[0].count) + 1);
    res.json({ message: `Card gifted to ${recipientUsername}!`, newAchievements: achList });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// ─── MYTHIC CRAFTING ─────────────────────────────────────────────
app.post('/api/crafting/mythic-forge', auth, async (req, res) => {
  try {
    const { cardIds } = req.body;
    if (!Array.isArray(cardIds) || cardIds.length !== 5) return res.status(400).json({ error: 'Select exactly 5 cards' });
    const ids = cardIds.map(Number);
    const owned = await query('SELECT card_id, quantity FROM user_cards WHERE user_id=$1 AND card_id=ANY($2)', [req.user.id, ids]);
    if (owned.rows.length < 5) return res.status(400).json({ error: 'You do not own all selected cards' });
    const cards = await query('SELECT id, type, rarity FROM cards WHERE id=ANY($1)', [ids]);
    const rarities = [...new Set(cards.rows.map(c => c.rarity))];
    const types    = [...new Set(cards.rows.map(c => c.type))];
    if (rarities.length !== 1 || rarities[0] !== 'Rare') return res.status(400).json({ error: 'All 5 cards must be Rare rarity' });
    if (types.length !== 1) return res.status(400).json({ error: 'All 5 cards must be the same type' });
    const cardType = types[0];
    for (const id of ids) {
      const row = owned.rows.find(r => r.card_id === id);
      if (row.quantity <= 1) await query('DELETE FROM user_cards WHERE user_id=$1 AND card_id=$2', [req.user.id, id]);
      else await query('UPDATE user_cards SET quantity=quantity-1 WHERE user_id=$1 AND card_id=$2', [req.user.id, id]);
    }
    let legendary = await query("SELECT * FROM cards WHERE rarity='Legendary' AND type=$1 ORDER BY RANDOM() LIMIT 1", [cardType]);
    if (!legendary.rows.length) {
      legendary = await query("SELECT * FROM cards WHERE rarity='Legendary' ORDER BY RANDOM() LIMIT 1");
      if (!legendary.rows.length) return res.status(500).json({ error: 'No Legendary cards found' });
    }
    const result = legendary.rows[0];
    await query('INSERT INTO user_cards (user_id,card_id) VALUES ($1,$2) ON CONFLICT (user_id,card_id) DO UPDATE SET quantity=user_cards.quantity+1', [req.user.id, result.id]);
    const achList = await checkAchievements(req.user.id, 'craft_legendary', 1);
    await logAction(req.user.id, 'MYTHIC_FORGE', null, `${cardType} x5 → ${result.name}`);
    res.json({ card: result, newAchievements: achList });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// ─── SCOUTING ─────────────────────────────────────────────────────
app.post('/api/pvp/scout', auth, async (req, res) => {
  try {
    const { targetUsername } = req.body;
    if (!targetUsername) return res.status(400).json({ error: 'targetUsername required' });
    const target = await query('SELECT id FROM users WHERE LOWER(username)=LOWER($1)', [targetUsername]);
    if (!target.rows.length) return res.status(404).json({ error: 'Player not found' });
    const targetId = target.rows[0].id;
    const user = await query('SELECT coins FROM users WHERE id=$1', [req.user.id]);
    if ((user.rows[0]?.coins || 0) < 50) return res.status(400).json({ error: 'Not enough coins (50 required)' });
    await query('UPDATE users SET coins=coins-50 WHERE id=$1', [req.user.id]);
    const cards = await getPlayerDeck(targetId);
    const top3 = cards.sort((a,b) => b.atk - a.atk).slice(0,3).map(c => ({
      name: c.name, type: c.type, rarity: c.rarity, atk: c.atk, hp: c.hp, def: c.def
    }));
    res.json({ cards: top3 });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/battle/start', auth, checkMaintenance('battle'), async (req, res) => {
  try {
    ...
    const battle = {
      ...
      playerVoidStored:     0,
      weather:              rollWeather(),
      playerRevengeActive:  false,
      playerRevengeTurns:   0,
    };

    const wMsgs = { storm: '⛈️ A storm brews — Thunder cards deal extra damage!', drought: '☀️ A drought scorches the field — Fire is empowered, Water is weakened!', fog: '🌫️ Thick fog rolls in — all attacks are weakened!' };
    if (battle.weather && wMsgs[battle.weather]) battle.log.push(wMsgs[battle.weather]);
...
app.post('/api/battle/action', auth, async (req, res) => {
  try {
    ...
    if (action === 'basic' || action === 'attack') {
      ...
      // Revenge bonus
      const revMult = battle.playerRevengeActive ? 1.2 : 1;
      if (battle.playerRevengeActive) {
        battle.playerRevengeTurns--;
        if (battle.playerRevengeTurns <= 0) { battle.playerRevengeActive = false; battle.playerRevengeTurns = 0; }
      }
      dmg = applyWeather(Math.floor(dmg * revMult), pa.type, battle.weather);
    ...
    if (action === 'ability') {
      ...
      let dmg = applyWeather(calcDamage(pa, aa), pa.type, battle.weather);
      // Revenge bonus for ability
      const revMult2 = battle.playerRevengeActive ? 1.2 : 1;
      if (battle.playerRevengeActive) {
        battle.playerRevengeTurns--;
        if (battle.playerRevengeTurns <= 0) { battle.playerRevengeActive = false; battle.playerRevengeTurns = 0; }
      }
      dmg = Math.floor(dmg * comboMult * bonus * boostMult2 * coachAtkMult2 * revMult2);
...
function battleView(b) {
  return {
    ...
    playerRevengeActive:  b.playerRevengeActive  || false,
    playerRevengeTurns:   b.playerRevengeTurns   || 0,
    newAchievements:      b.newAchievements      || [],
  };
}
...
async function finishBattle(battle, userId, won) {
  try {
    ...
    // Achievement checks
    const totalWins = await query('SELECT wins FROM ranked_stats WHERE user_id=$1', [userId]);
    const wins = (totalWins.rows[0]?.wins || 0) + (won ? 1 : 0);
    const [achWin, achPlay] = await Promise.all([
      won ? checkAchievements(userId, 'win_battle', wins) : Promise.resolve([]),
      checkAchievements(userId, 'play_battle', wins + (won ? 0 : 1)),
    ]);
    battle.newAchievements = [...(achWin||[]), ...(achPlay||[])];
  } catch (err) {
...
// ─── ADMIN: ECONOMY ANALYTICS ────────────────────────────────────
app.get('/api/admin/economy/analytics', auth, requireRole('admin'), async (req, res) => {
  try {
    const [coins, trades, users, dailyUsers, packs] = await Promise.all([
      query('SELECT SUM(coins) as total, AVG(coins)::int as avg, MAX(coins) as max, MIN(coins) as min FROM users WHERE NOT banned'),
      query("SELECT COUNT(*) as total, COUNT(CASE WHEN status='completed' THEN 1 END) as completed FROM trades"),
      query('SELECT COUNT(*) as total, COUNT(CASE WHEN created_at > NOW()-INTERVAL \'7 days\' THEN 1 END) as new_week FROM users'),
      query("SELECT DATE(created_at)::text as day, COUNT(*) as cnt FROM users WHERE created_at > NOW()-INTERVAL '14 days' GROUP BY day ORDER BY day DESC"),
    ...
    res.json({
      ...
      users: users.rows[0],
      dailySignups: dailyUsers.rows,
      totalMatches: parseInt(packs.rows[0].total),
    });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// ─── ADMIN: ACTIVE BATTLES ────────────────────────────────────────
app.get('/api/admin/battles/active', auth, requireRole('admin'), (req, res) => {
  try {
    const ai = [...activeBattles.values()].map(b => ({
      userId: b.userId, turns: b.log.length, elapsed: Math.floor((Date.now() - (b.startedAt||b.createdAt))/1000),
      finished: b.finished, weather: b.weather,
    }));
    const pvp = [...pvpBattles.values()].map(b => ({
      id: b.id, p1: b.player1Username, p2: b.player2Username, turn: b.turn,
      ranked: b.ranked, elapsed: Math.floor((Date.now() - b.createdAt)/1000),
      finished: b.finished, weather: b.weather,
    }));
    res.json({ ai, pvp, queueSize: pvpQueue.size });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// ─── ADMIN: AUDIT LOG WITH FILTERS ────────────────────────────────
app.get('/api/admin/logs/filtered', auth, requireRole('mod'), async (req, res) => {
  try {
    const { action, adminName, targetId, from, to, limit = 200 } = req.query;
    const where = [];
    const params = [];
    let idx = 1;
    if (action)    { where.push(`al.action ILIKE $${idx++}`);              params.push('%' + action + '%'); }
    if (adminName) { where.push(`u.username ILIKE $${idx++}`);             params.push('%' + adminName + '%'); }
    if (targetId)  { where.push(`al.target_user_id=$${idx++}`);            params.push(parseInt(targetId)); }
    if (from)      { where.push(`al.created_at >= $${idx++}`);             params.push(new Date(from)); }
    if (to)        { where.push(`al.created_at <= $${idx++}`);             params.push(new Date(to + 'T23:59:59')); }
    const whereStr = where.length ? 'WHERE ' + where.join(' AND ') : '';
    params.push(Math.min(parseInt(limit), 500));
    const r = await query(
      `SELECT al.*, u.username as admin_name FROM admin_logs al LEFT JOIN users u ON u.id=al.admin_id ${whereStr} ORDER BY al.created_at DESC LIMIT $${idx}`,
      params
    );
    res.json(r.rows);
  } catch(e) { res.status(500).json({ error: e.message }); }
...
app.post('/api/admin/users/bulk-ban', auth, requireRole('owner'), async (req, res) => {
  try {
    const { userIds, reason } = req.body;
    if (!Array.isArray(userIds) || !userIds.length) return res.status(400).json({ error: 'userIds required' });
    for (const uid of userIds) {
      await query('UPDATE users SET banned=true, ban_reason=$1 WHERE id=$2', [reason || 'Bulk ban', parseInt(uid)]);
      await logAction(req.user.id, 'BULK_BAN', uid, reason || 'Bulk ban');
    }
    res.json({ message: `Banned ${userIds.length} users` });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/admin/users/bulk-coins', auth, requireRole('owner'), async (req, res) => {
  try {
    ...
    for (const uid of userIds) {
      await query('UPDATE users SET coins=GREATEST(0,coins+$1) WHERE id=$2', [amt, parseInt(uid)]);
    }
    await logAction(req.user.id, 'BULK_COINS', null, `${userIds.length} users, ${amt} coins each`);
    res.json({ message: `Applied ${amt} coins to ${userIds.length} users` });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
// ─── DEV: ACTIVE BATTLES ─────────────────────────────────────────
app.get('/api/dev/battles', ...devAuth, (req, res) => {
  res.json({
    ai: [...activeBattles.values()].map(b => ({ userId: b.userId, turns: b.log.length, weather: b.weather, finished: b.finished, elapsed: Math.floor((Date.now()-(b.startedAt||b.createdAt))/1000) })),
    pvp: [...pvpBattles.values()].map(b => ({ id: b.id, p1: b.player1Username, p2: b.player2Username, turn: b.turn, weather: b.weather, ranked: b.ranked, finished: b.finished, elapsed: Math.floor((Date.now()-b.createdAt)/1000) })),
    queue: [...pvpQueue.values()].map(q => ({ userId: q.userId, username: q.username, ranked: q.ranked })),
    poolStats: { total: pool.totalCount, idle: pool.idleCount, waiting: pool.waitingCount },
  });
});

app.delete('/api/dev/battles/ai/:userId', ...devAuth, async (req, res) => {
  try {
    const uid = parseInt(req.params.userId);
    const battle = activeBattles.get(uid);
    if (!battle) return res.status(404).json({ error: 'No AI battle found for that user' });
    battle.finished = true; battle.winner = 'ai';
    battle.log.push('[Dev] Battle force-ended by developer.');
    activeBattles.delete(uid);
    await logAction(req.user.id, 'DEV_FORCE_END_AI', uid, '');
    res.json({ message: 'AI battle ended' });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.delete('/api/dev/battles/pvp/:battleId', ...devAuth, async (req, res) => {
  try {
    const battle = pvpBattles.get(req.params.battleId);
    if (!battle) return res.status(404).json({ error: 'PvP battle not found' });
    battle.finished = true; battle.winner = 'player1';
    battle.log.push('[Dev] Battle force-ended by developer.');
    await finishPvpBattle(battle).catch(()=>{});
    setTimeout(() => { pvpBattles.delete(req.params.battleId); userToBattle.delete(battle.player1Id); userToBattle.delete(battle.player2Id); }, 5000);
    await logAction(req.user.id, 'DEV_FORCE_END_PVP', null, req.params.battleId);
    res.json({ message: 'PvP battle ended' });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// ─── DEV: POOL STATS ─────────────────────────────────────────────
app.get('/api/dev/pool', ...devAuth, (req, res) => {
