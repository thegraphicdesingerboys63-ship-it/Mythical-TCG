...
app.put('/api/admin/users/:id/ban', auth, requireRole('mod'), async (req, res) => {
  try {
    ...
    if (t.role === 'developer') return res.status(403).json({ error: 'Developer accounts cannot be banned' });
...
app.put('/api/admin/users/:id/role', auth, requireRole('admin'), async (req, res) => {
  try {
    ...
    if (!target.rows.length) return res.status(404).json({ error: 'User not found' });
    if (target.rows[0].role === 'developer') return res.status(403).json({ error: 'Developer accounts cannot have their role changed' });
