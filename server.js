...
app.post('/api/packs/open', auth, checkMaintenance('packs'), async (req, res) => {
  try {
    ...
    // 10% chance to drop a coach
    let coachDrop = null;
    if (Math.random() < 0.10) {
      const r = Math.random();
      let coachRarity;
      if (r < 0.60)      coachRarity = 'Common';
      else if (r < 0.85) coachRarity = 'Rare';
      else if (r < 0.97) coachRarity = 'Epic';
      else               coachRarity = 'Legendary';
      const coachRes = await query('SELECT * FROM coaches WHERE rarity=$1 ORDER BY RANDOM() LIMIT 1', [coachRarity]);
      if (coachRes.rows.length) {
        ...
        coachDrop = { id: ch.id, name: ch.name, portrait: ch.portrait, rarity: ch.rarity, description: ch.description };
      }
    }

    res.json({ cards: pulled, coachDrop });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/coaches/equip/:id', auth, async (req, res) => {
  try {
    ...
    // Verify ownership (or unequip if 0)
    if (coachId === 0) {
