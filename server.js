...
function calcBasicDamage(attacker, defender) {
  let mult = 1;
  if (attacker.type === defender.weakness)   mult = 2;
  if (attacker.type === defender.resistance) mult = 0.5;
...
function runAiTurn(battle) {
  const pa = battle.playerCards[battle.playerActive];
  // AI switches if weak to player type — resets AI orbs
  if (battle.aiCards[battle.aiActive].weakness === pa.type) {
    ...
    if (better !== -1) {
      battle.aiOrbs[battle.aiCards[better].type] = 0;
  ...
  if (aiActive.current_hp <= 0) return;
  ...
  if (aiActive.status?.type === 'paralysis') {
    ...
    if (Math.random() < 0.4) { battle.log.push(`⚡ Foe's ${aiActive.name} is paralyzed and can't move!`); return; }
  }
  ...
  if ((battle.aiOrbs[aiActive.type] || 0) >= cost) {
    battle.aiOrbs[aiActive.type] = (battle.aiOrbs[aiActive.type] || 0) - cost;
    let dmg = calcDamage(aiActive, pa);
    if (battle.bossSurgeActive) { dmg = Math.floor(dmg * 1.5); battle.bossSurgeActive = false; }
    dmg = Math.floor(dmg * speedBonus);
    ...
    battle.aiOrbs[aiActive.type] = (battle.aiOrbs[aiActive.type] || 0) + 1;
    const { dmg: rawDmg, crit } = calcBasicDamage(aiActive, pa);
    let dmg = Math.floor(rawDmg * speedBonus);
    if (battle.playerGuarded) { dmg = Math.floor(dmg * 0.5); battle.log.push(`🛡️ Your guard absorbed half the damage!`); }
    pa.current_hp = Math.max(0, pa.current_hp - dmg);
...
app.post('/api/packs/open', auth, async (req, res) => {
  try {
    ...
    const PACK_CONFIG = {
      ...
      rare:   { cost: 750,  count: 5  },
      ultra:  { cost: 2500, count: 7  },
      mythic: { cost: 8000, count: 10 },
    };
...
app.post('/api/battle/start', auth, async (req, res) => {
  try {
    ...
    const toSlot = cards => cards.map(c => ({ ...c, current_hp: c.hp, status: null }));
    ...
    const battle = {
      ...
      aiActive:     0,
      playerTurn:   true,
      log:          ['The battle begins! Use Quick Strike to build orbs, then unleash your Ability!'],
      finished:     false,
      winner:       null,
      createdAt:    Date.now(),
      playerOrbs:      {},
...
app.post('/api/battle/action', auth, async (req, res) => {
  try {
    ...
    if (action === 'switch') {
      ...
      const switchType = activeCard.type;
      if ((battle.playerOrbs[switchType] || 0) < switchCost)
        return res.status(400).json({ error: `Retreating ${activeCard.name} costs ${switchCost} ${switchType} orbs. You have ${battle.playerOrbs[switchType] || 0}.` });
      battle.playerOrbs[switchType] = (battle.playerOrbs[switchType] || 0) - switchCost;
      battle.playerCombo = 0;
      battle.log.push(`You switched to ${battle.playerCards[idx].name}! (-${switchCost} ${switchType} orb${switchCost > 1 ? 's' : ''})`);
    ...
    if (action === 'basic' || action === 'attack') {
      const orbGain = pa.spd >= aa.spd * 1.3 ? 2 : 1;
      battle.playerOrbs[pa.type] = (battle.playerOrbs[pa.type] || 0) + orbGain;
      ...
      const dmg = Math.floor(rawDmg * bonus);
      ...
      parts.push(`(+${orbGain} ${pa.type} orb${orbGain > 1 ? 's' : ''} → ${battle.playerOrbs[pa.type]})`);
      if (orbGain > 1) parts.push('⚡ Speed boost!');
    ...
    if (action === 'ability') {
      ...
      if ((battle.playerOrbs[pa.type] || 0) < cost)
        return res.status(400).json({ error: `Need ${cost} ${pa.type} orbs. You have ${battle.playerOrbs[pa.type] || 0}.` });
      battle.playerOrbs[pa.type] = (battle.playerOrbs[pa.type] || 0) - cost;
      const combo = battle.playerCombo || 0;
      const comboMult = combo >= 3 ? 1.5 : 1;
      battle.playerCombo = 0;
      const bonus = getBenchBonus(battle.playerCards, battle.playerActive);
      let dmg = calcDamage(pa, aa);
      dmg = Math.floor(dmg * comboMult * bonus);
  ...
  try {
    ...
    if (battle.isConquest) {
      if (won) {
        ...
        // Set result immediately so win is always registered even if piece queries fail
        battle.ratingResult = { conquestWin: true, coinsEarned: reward, bossCardUnlocked: null, pieceDropped: stId };
...
app.post('/api/conquest/start', auth, async (req, res) => {
  try {
    ...
    const battle = {
      ...
      aiActive:          0,
      playerTurn:        true,
      log:               [stage.isBoss ? '⚔️ BOSS BATTLE! Build orbs with Quick Strike, then unleash your Ability!' : 'Conquest battle! Strike to build orbs, then use your Ability for full power.'],
      finished:          false,
      winner:            null,
      createdAt:         Date.now(),
      isConquest:        true,
...
app.post('/api/dev/remove-card', ...devAuth, async (req, res) => {
  try {
    const { card_id, username } = req.body;
    if (!card_id || !username) return res.status(400).json({ error: 'card_id and username required' });
    const u = await query('SELECT id, username FROM users WHERE LOWER(username)=LOWER($1)', [username]);
    if (!u.rows.length) return res.status(404).json({ error: `User "${username}" not found` });
