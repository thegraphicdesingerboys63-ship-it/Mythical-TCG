...
app.post('/api/battle/start', auth, checkMaintenance('battle'), async (req, res) => {
  try {
    ...
    const toSlot = cards => cards.map(c => ({ ...c, current_hp: c.hp, status: null, orbs: 0 }));
    ...
    const battle = {
      ...
      aiActive:     0,
      playerTurn:   true,
      log:          ['The battle begins! Attach Energy each turn to power up your cards, then strike!'],
      finished:     false,
      winner:       null,
      createdAt:    Date.now(),
      playerGuarded:        false,
...
app.post('/api/battle/action', auth, async (req, res) => {
  try {
    ...
    if (action === 'basic' || action === 'attack') {
      ...
      const dmg = Math.floor(rawDmg * bonus * speedBonus);
