  ...
  try {
    ...
    if (battle.isConquest) {
      if (won) {
        ...
        // Boss card unlock: give player the boss card when they beat a boss stage
        let bossCardUnlocked = null;
        if (battle.isBoss) {
          const key = `${battle.conquestChapterId}_${battle.conquestStageId}`;
          ...
          if (bc) {
            ...
            bossCardUnlocked = bc.name;
          }
        }
        battle.ratingResult = { conquestWin: true, coinsEarned: reward, bossCardUnlocked };
      } else {
...
// Give card to self — developer only, always targets req.user.id (no other user can be targeted)
app.post('/api/dev/self/give', ...devAuth, async (req, res) => {
  try {
    const { card_id, card_name } = req.body;
    let cardRow;
    if (card_id) {
    ...
    await query(
      ...
      [req.user.id, cardRow.id]
    );
    await logAction(req.user.id, 'DEV_SELF_GIVE', req.user.id, cardRow.name);
    res.json({ message: `Card "${cardRow.name}" added to your collection`, card: cardRow });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/dev/cards/search', ...devAuth, async (req, res) => {
  try {
    ...
    const r = await query(
      `SELECT id, name, type, rarity, class FROM cards WHERE LOWER(name) LIKE LOWER($1) ORDER BY name LIMIT 20`,
      [`%${q}%`]
    );
    res.json(r.rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
