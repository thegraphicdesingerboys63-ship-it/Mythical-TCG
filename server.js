...
app.post('/api/trades/:id/accept', auth, async (req, res) => {
  const client = await (await import('./db.js')).pool?.connect?.() ?? null;
  try {
...
app.post('/api/battle/action', auth, async (req, res) => {
  try {
    ...
    if (action === 'heal') {
      ...
      const healCost = 2;
      if ((battle.playerHealUses || 0) >= 2)
        return res.status(400).json({ error: 'Heal used 2 times already — limit reached.' });
      ...
      battle.playerCombo = 0;
      battle.log.push(`💚 ${pa3.name} recovered ${healAmt} HP! (${battle.playerHealUses}/2 heals used)`);
      battle.playerTurn = false;
      runAiTurn(battle);
      advanceFainted(battle);
      if (!checkWin(battle)) battle.playerTurn = true;
    ...
    if (action === 'basic' || action === 'attack') {
      ...
      const dmg = Math.floor(rawDmg * bonus * speedBonus * boostMult);
    ...
    if (action === 'ability') {
      ...
      dmg = Math.floor(dmg * comboMult * bonus * boostMult2);
