...
async function initDB() {
  ...
  // ─── SEED COACHES ───────────────────────────────────────────────
  const coachCount = await query('SELECT COUNT(*) FROM coaches');
  if (parseInt(coachCount.rows[0].count) === 0) {
    const coaches = [
      ...
      ['Captain Lyn',      '⚔️', 'crit_bonus',  0.08, 'Rare',      ['Find the gap!', 'Strike their weakness!', 'Hit where it hurts!', 'Precision is everything!'],   '+8% Critical hit chance'],
    ...
    for (const [name, portrait, buff_type, buff_value, rarity, quotes, description] of coaches) {
      await query(
        'INSERT INTO coaches (name, portrait, buff_type, buff_value, rarity, quote_lines, description) VALUES ($1,$2,$3,$4,$5,$6,$7)',
...
async function seedCards() {
  ...
  if (count === 200) return;
  if (count > 0) {
    console.log(`Card count is ${count}, reseeding to 200...`);
    await query('DELETE FROM user_cards');
    await query('DELETE FROM cards');
  }
  console.log('Seeding 200 cards...');
  const BATCH = 50;
