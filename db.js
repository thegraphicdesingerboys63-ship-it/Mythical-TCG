...
async function seedAdmin() {
  ...
  const existing = await query("SELECT id FROM users WHERE username = 'AMGProdZ'");
  ...
  const res = await query(
    ...
    ['AMGProdZ', hash, 'developer', 999999]
  );
  const uid = res.rows[0].id;
  await query('INSERT INTO user_settings (user_id) VALUES ($1) ON CONFLICT DO NOTHING', [uid]);
  await query('INSERT INTO ranked_stats (user_id, rating, rank_title) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING', [uid, 9999, 'Developer']);
  console.log('Developer account AMGProdZ created.');
}
