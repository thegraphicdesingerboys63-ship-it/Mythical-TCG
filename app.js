...
function _cqPanelArt(chId, stId, panelIdx) {
  ...
  const PAL = {
    1: ['#2da84a','#1a5c22','#0d2a09','#030d02'],
    2: ['#8b3fc8','#4a1a70','#1a0836','#060210'],
    3: ['#2980b9','#1a4a7a','#020e1e','#010610'],
    4: ['#e67e22','#8b2000','#3a0800','#0d0200'],
    5: ['#74b9ff','#2a6aaa','#02101a','#010810'],
    6: ['#6c5ce7','#4a3aa0','#0a0820','#020210'],
    7: ['#a29bfe','#6c5ce7','#0d0d1a','#010104'],
    8: ['#ff4466','#8b0022','#200010','#060000'],
    9: ['#b8a0f8','#7c6ad0','#12082a','#05030e'],
    10:['#ff2d6b','#aa0040','#200010','#070002'],
  };
  const [accent, mid, deep, bg] = PAL[chId] || PAL[1];
  const seed = chId * 17 + stId * 7 + panelIdx * 3;
  const intensity = {calm:0.35, tense:0.65, dark:0.85, boss:1.1}[mood] || 0.6;

  // Atmospheric particles
  const pCount = Math.floor(18 + intensity * 32);
  const particles = Array.from({length: pCount}, (_, i) => {
    const x = (i * 43 + seed * 13) % 480;
    const y = (i * 29 + seed * 7)  % 160;
    const r = 0.4 + (i % 3) * 0.5;
    const op = ((0.04 + (i % 5) * 0.018) * intensity).toFixed(3);
    return `<circle cx="${x}" cy="${y}" r="${r}" fill="${accent}" opacity="${op}"/>`;
  }).join('');

  // Chapter + stage specific scene element
  let scene = '';
  switch (chId) {
    case 1: { // Earth forest — trees, roots, corruption spreading
      const tCount = 7 + stId * 2;
      const trees = Array.from({length: tCount}, (_, i) => {
        const x = 25 + i * Math.floor(430 / tCount);
        const h = 35 + (i % 5) * 18 + stId * 8;
        const corruptOp = mood === 'boss' || stId >= 3 ? 0.5 : mood === 'dark' ? 0.25 : 0;
        return `<rect x="${x-3}" y="${120-h}" width="6" height="${h}" fill="${accent}" opacity="${0.13+i%4*0.04}" rx="2"/>
          ${corruptOp > 0 ? `<rect x="${x-3}" y="${120-h}" width="6" height="${h*0.4}" fill="${deep}" opacity="${corruptOp}" rx="2"/>` : ''}`;
      }).join('');
      const roots = stId >= 2 ? Array.from({length: 5 + panelIdx * 2}, (_, i) =>
        `<path d="M${100+i*60},120 Q${125+i*55},${95+i*4} ${85+i*70},${72+stId*7}" stroke="${mid}" stroke-width="${1+i%2}" fill="none" opacity="${0.25+i*0.04}"/>`
      ).join('') : '';
      const voidGlass = mood === 'boss' ? `<ellipse cx="240" cy="118" rx="130" ry="10" fill="${deep}" opacity="0.9"/>` : '';
      scene = trees + roots + voidGlass;
      break;
    }
    case 2: { // Shadow wilds — wolves, void tendrils
      const wolves = Array.from({length: Math.min(stId + panelIdx, 4)}, (_, i) => {
        const x = 70 + i * 110; const y = 92 + (i % 2) * 12; const s = 0.75 + i * 0.12;
        return `<ellipse cx="${x}" cy="${y}" rx="${22*s}" ry="${13*s}" fill="${deep}" opacity="0.75"/>
          <ellipse cx="${x-9*s}" cy="${y-9*s}" rx="${7*s}" ry="${5*s}" fill="${deep}" opacity="0.65"/>
          <circle cx="${x-13*s}" cy="${y-11*s}" r="${3*s}" fill="${accent}" opacity="${0.5+i*0.08}"/>`;
      }).join('');
      const tendrils = Array.from({length: 5 + stId * 2}, (_, i) =>
        `<path d="M${(i*79+20)%480},160 Q${(i*47+35)%480},${115-i*7} ${(i*61+15)%480},${75-stId*8}" stroke="${mid}" stroke-width="${1+i%3}" fill="none" opacity="${0.18+i%4*0.04}"/>`
      ).join('');
      scene = tendrils + wolves;
      break;
    }
    case 3: { // Water lake — waves, depth, dragon
      const waves = Array.from({length: 7}, (_, i) =>
        `<path d="M${i*68},${88+stId*4} Q${i*68+34},${78+stId*4} ${i*68+68},${88+stId*4}" stroke="${accent}" stroke-width="1.5" fill="none" opacity="${0.18+i%3*0.04}"/>`
      ).join('');
      const depth = `<ellipse cx="240" cy="${118+stId*4}" rx="210" ry="${28+stId*8}" fill="${mid}" opacity="0.22"/>`;
      const dragon = stId >= 3
        ? `<path d="M40,118 Q130,${55-panelIdx*8} 220,${38+stId*4} Q300,${65+stId*2} 380,${85+stId} Q430,92 480,100" stroke="${accent}" stroke-width="${3+panelIdx}" fill="none" opacity="0.38"/>
           <circle cx="220" cy="${38+stId*4}" r="${9+panelIdx*4}" fill="${accent}" opacity="0.28"/>`
        : `<ellipse cx="${190+panelIdx*50}" cy="${50+stId*10}" rx="${18+stId*6}" ry="${12+stId*4}" fill="${mid}" opacity="0.18"/>`;
      scene = waves + depth + dragon;
      break;
    }
    case 4: { // Fire citadel — towers, ash, dark flame
      const towers = Array.from({length: 5}, (_, i) =>
        `<rect x="${45+i*88}" y="${18+i%2*18}" width="16" height="${78-i%2*8}" fill="${deep}" opacity="0.82" rx="1"/>
         <rect x="${49+i*88}" y="${14+i%2*18}" width="8" height="9" fill="${mood==='boss'?deep:mid}" opacity="${stId<3?0.55:0.15}" rx="1"/>`
      ).join('');
      const ashParticles = Array.from({length: 12 + stId * 4}, (_, i) =>
        `<circle cx="${(i*53+seed*9)%480}" cy="${(i*19+seed*5)%140+10}" r="${0.7+i%3*0.5}" fill="${accent}" opacity="${0.04+i%5*0.025}"/>`
      ).join('');
      const flame = stId >= 2 ? Array.from({length: 5 + panelIdx * 2}, (_, i) =>
        `<path d="M${170+i*30},128 Q${178+i*28},${90-stId*7+i*4} ${188+i*22},${68-stId*5}" stroke="${accent}" stroke-width="${2+i%2}" fill="none" opacity="${0.18+i%3*0.05}"/>`
      ).join('') : '';
      scene = towers + ashParticles + flame;
      break;
    }
    case 5: { // Ice highlands — crystal spikes, throne
      const spikes = Array.from({length: 9 + stId * 2}, (_, i) => {
        const x = 28 + i * 45 + (i % 3 - 1) * 8;
        const h = 28 + (i % 5) * 18 + stId * 6;
        return `<polygon points="${x},160 ${x-7},${160-h} ${x+7},${160-h}" fill="${accent}" opacity="${0.1+i%4*0.035}"/>`;
      }).join('');
      const throne = stId >= 3
        ? `<rect x="202" y="58" width="76" height="62" fill="${deep}" opacity="0.72" rx="3"/>
           <rect x="214" y="40" width="52" height="26" fill="${mid}" opacity="0.5" rx="2"/>
           <circle cx="240" cy="53" r="${8+panelIdx*4}" fill="${accent}" opacity="${0.25+panelIdx*0.08}"/>`
        : '';
      const gridLines = Array.from({length: 7}, (_, i) =>
        `<line x1="${i*68}" y1="0" x2="${i*68+28}" y2="160" stroke="${mid}" stroke-width="0.4" opacity="0.08"/>`
      ).join('');
      scene = gridLines + spikes + throne;
      break;
    }
    case 6: { // Cosmic rift — stars, crack
      const stars = Array.from({length: 40 + stId * 8}, (_, i) =>
        `<circle cx="${(i*37+seed*11)%480}" cy="${(i*23+seed*7)%160}" r="${0.3+i%4*0.35}" fill="${accent}" opacity="${0.07+i%7*0.025}"/>`
      ).join('');
      const riftX = 230 + panelIdx * 8;
      const crack = `<line x1="${riftX}" y1="0" x2="${riftX+10}" y2="160" stroke="${accent}" stroke-width="${1+stId*0.4}" opacity="${0.28+stId*0.08}"/>
        <line x1="${riftX+14}" y1="0" x2="${riftX+4}" y2="160" stroke="${mid}" stroke-width="${0.5+stId*0.25}" opacity="${0.18+stId*0.06}"/>`;
      const portal = stId >= 3
        ? `<ellipse cx="240" cy="80" rx="${28+panelIdx*14}" ry="${18+panelIdx*9}" fill="none" stroke="${accent}" stroke-width="2" opacity="0.38"/>
           <ellipse cx="240" cy="80" rx="${14+panelIdx*7}" ry="${9+panelIdx*4}" fill="${deep}" opacity="0.45"/>`
        : '';
      scene = stars + crack + portal;
      break;
    }
    case 7: { // Void spire — geometric crystals, tower
      const crystals = Array.from({length: 7 + stId * 2}, (_, i) => {
        const x = (i * 73 + seed * 11) % 460 + 10;
        const y = (i * 41 + seed * 9) % 110 + 20;
        const s = 4 + i % 4 * 5;
        return `<polygon points="${x},${y-s} ${x+s},${y+s} ${x},${y+s*0.4} ${x-s},${y+s}" fill="${mid}" opacity="${0.13+i%3*0.045}"/>`;
      }).join('');
      const spireH = 95 + stId * 12;
      const spire = `<rect x="227" y="${20-stId*4}" width="26" height="${spireH}" fill="${deep}" opacity="0.82" rx="2"/>
        <polygon points="233,${20-stId*4} 240,${4-stId*3} 247,${20-stId*4}" fill="${accent}" opacity="${0.28+panelIdx*0.08}"/>`;
      const geoLines = Array.from({length: 5}, (_, i) =>
        `<line x1="${95+i*70}" y1="${35+i*12}" x2="${135+i*58}" y2="${98+i*8}" stroke="${accent}" stroke-width="0.4" opacity="0.16"/>`
      ).join('');
      scene = crystals + geoLines + spire;
      break;
    }
    case 8: { // Grief, forgotten cards
      const cardCount = 3 + panelIdx * 2 + stId;
      const cards = Array.from({length: Math.min(cardCount, 10)}, (_, i) => {
        const x = 55 + i * 42 + (i % 3 - 1) * 16;
        const y = 48 + (i % 3) * 18;
        const rot = (i % 5 - 2) * 7;
        const op = mood === 'boss' ? 0.55 : 0.22 + i * 0.04;
        return `<rect x="${x}" y="${y}" width="30" height="44" fill="${mid}" opacity="${op}" rx="3" transform="rotate(${rot},${x+15},${y+22})"/>
          <rect x="${x+3}" y="${y+3}" width="24" height="38" fill="${bg}" opacity="${op*0.5}" rx="2" transform="rotate(${rot},${x+15},${y+22})"/>`;
      }).join('');
      const grief = Array.from({length: 15 + stId * 6}, (_, i) =>
        `<circle cx="${(i*31+seed*13)%480}" cy="${(i*19+seed*5)%160}" r="${0.4+i%3*0.4}" fill="${accent}" opacity="${0.04+i%5*0.018}"/>`
      ).join('');
      const pool = `<ellipse cx="240" cy="142" rx="${115+stId*18}" ry="${18+stId*4}" fill="${deep}" opacity="${0.48+panelIdx*0.12}"/>`;
      scene = grief + cards + pool;
      break;
    }
    case 9: { // Spirit realm — wisps, gate, veil
      const wisps = Array.from({length: 10 + stId * 3}, (_, i) => {
        const x = (i * 71 + seed * 13) % 460 + 10;
        const y = (i * 37 + seed * 9) % 130 + 10;
        return `<ellipse cx="${x}" cy="${y}" rx="${7+i%5*4}" ry="${4+i%4*3}" fill="${accent}" opacity="${0.04+i%5*0.018}"/>
          <circle cx="${x}" cy="${y}" r="${1.5+i%3}" fill="${accent}" opacity="${0.05+i%4*0.02}"/>`;
      }).join('');
      const gate = stId >= 3
        ? `<rect x="192" y="28" width="96" height="108" fill="none" stroke="${accent}" stroke-width="2" opacity="${0.28+panelIdx*0.1}" rx="3"/>
           <rect x="206" y="28" width="68" height="108" fill="${deep}" opacity="${0.38+panelIdx*0.1}" rx="2"/>
           <circle cx="240" cy="78" r="${14+panelIdx*8}" fill="${accent}" opacity="${0.06+panelIdx*0.035}"/>`
        : `<circle cx="240" cy="${65+stId*9}" r="${18+stId*9+panelIdx*5}" fill="${mid}" opacity="0.09"/>`;
      const veilLines = Array.from({length: 5}, (_, i) =>
        `<line x1="${80+i*80}" y1="0" x2="${75+i*82}" y2="160" stroke="${accent}" stroke-width="0.4" opacity="0.08"/>`
      ).join('');
      scene = veilLines + wisps + gate;
      break;
    }
    case 10: { // God's descent — city, beam, agents
      const walls = Array.from({length: 7}, (_, i) =>
        `<rect x="${i*70}" y="${78+i%2*14}" width="62" height="${46+i%3*9}" fill="${deep}" opacity="0.72" rx="1"/>
         <rect x="${i*70+4}" y="${74+i%2*14}" width="54" height="10" fill="${mid}" opacity="0.45"/>`
      ).join('');
      const beamX = 240 + panelIdx * 8;
      const beam = `<line x1="${beamX}" y1="0" x2="${beamX+panelIdx*6}" y2="160" stroke="${accent}" stroke-width="${2+panelIdx}" opacity="${0.13+stId*0.04}"/>
        <ellipse cx="${beamX+panelIdx*3}" cy="${35+stId*9}" rx="${18+panelIdx*14}" ry="${12+panelIdx*9}" fill="${accent}" opacity="${0.05+panelIdx*0.025}"/>`;
      const agents = stId >= 2 ? Array.from({length: stId * 2 + panelIdx + 1}, (_, i) => {
        const x = 65 + i * 55;
        return `<ellipse cx="${x}" cy="100" rx="7" ry="18" fill="${mid}" opacity="${0.38+i*0.04}"/>
          <circle cx="${x}" cy="76" r="6" fill="${deep}" opacity="0.72"/>
          <circle cx="${x-2}" cy="74" r="2" fill="${accent}" opacity="0.58"/>`;
      }).join('') : '';
  ...
  const vignOp = (0.08 + intensity * 0.22).toFixed(3);
  const bottomFade = `<rect x="0" y="95" width="480" height="65" fill="${bg}" opacity="0.55"/>`;

  return `<svg viewBox="0 0 480 160" xmlns="http://www.w3.org/2000/svg" class="cq-panel-art-svg">
    <defs>
      <radialGradient id="pa-vign-${chId}" cx="50%" cy="50%" r="70%">
        <stop offset="0%" stop-color="transparent"/>
        <stop offset="100%" stop-color="${bg}" stop-opacity="${vignOp}"/>
      </radialGradient>
    </defs>
    ...
    ${scene}
    ${bottomFade}
    <rect width="480" height="160" fill="url(#pa-vign-${chId})" opacity="0.8"/>
  </svg>`;
}
...
function _cqStartPanel(idx) {
  ...
  if (artEl) { artEl.classList.remove('cq-art-in'); void artEl.offsetWidth; artEl.innerHTML = _cqPanelArt(s.ch.id, s.st.id, idx); artEl.classList.add('cq-art-in'); }
    ...
    if (data.finished && data.ratingResult) {
      ...
      if (r.conquestWin !== undefined) {
        ...
        if (r.conquestWin) {
          ...
          notify(`Victory! +${r.coinsEarned} coins.${pieceMsg}`, 'success');
...
function viewFriends() {
  ...
  const friendList = accepted.length
    ? accepted.map(f => {
        ...
        <button class="btn btn-sm btn-primary dm-chat-btn" onclick="openChat(${f.other_user_id},'${f.username}')">
          💬 Chat${unread ? `<span class="dm-badge">${unread}</span>` : ''}
        </button>
...
function adminDeveloper() {
  return `<div class="sketch-box" style="border-color:var(--red)">
    ...
    <p class="text-muted mb-1" style="font-size:0.82rem">Search a card, enter the target username, then hit Remove.</p>
    <div style="display:flex;gap:0.6rem;margin-bottom:0.5rem;flex-wrap:wrap">
      <input class="input-box" id="dev-rm-search" placeholder="Search cards..." style="flex:2;min-width:160px"
        onkeydown="if(event.key==='Enter')devRmCardSearch()">
      <input class="input-box" id="dev-rm-target" placeholder="Username (required)" style="flex:1;min-width:130px">
      <button class="btn btn-red" onclick="devRmCardSearch()">Search</button>
    </div>
    <div id="dev-rm-results" style="display:flex;flex-direction:column;gap:0.35rem;margin-bottom:0.5rem"></div>
  ...
  try {
    ...
    resultsEl.innerHTML = cards.map(c =>
      `<div style="display:flex;align-items:center;gap:0.6rem;background:var(--navy-light);border:1px solid var(--navy-border);border-radius:4px;padding:0.4rem 0.8rem;font-size:0.85rem">
        <span style="color:${typeColor(c.type)};font-size:0.78rem;min-width:50px">${c.type}</span>
        <span style="flex:1"><strong>${c.name}</strong> <span class="text-muted" style="font-size:0.78rem">#${c.id} · ${c.rarity}${c.set_name ? ' · '+c.set_name : ''}</span></span>
        <button class="btn btn-sm btn-red" style="padding:0.2rem 0.7rem;font-size:0.8rem;white-space:nowrap" onclick="devRemoveCard(${c.id})">Remove</button>
      </div>`
    ).join('');
  } catch(e) { if (out) out.innerHTML = `<span style="color:var(--red)">${e.message}</span>`; }
};

window.devRemoveCard = async (cardId) => {
  const out = document.getElementById('dev-rm-out');
  const target = document.getElementById('dev-rm-target')?.value?.trim();
  if (!target) { notify('Username required to remove a card', 'error'); return; }
  try {
    const r = await api('/dev/remove-card', 'POST', { card_id: cardId, username: target });
    if (out) out.innerHTML = `<span style="color:var(--teal)">✓ ${r.message}</span>`;
    notify(r.message, 'success');
  } catch(e) {
    if (out) out.innerHTML = `<span style="color:var(--red)">${e.message}</span>`;
  ...
  if (S.user) {
    ...
    setInterval(async () => {
      ...
      if (friends.value) {
        ...
        if (S.view === 'friends') { document.getElementById('page').innerHTML = viewFriends(); attachListeners(); }
      }
      ...
      if (dmUnread.value) {
        ...
        if (totalNow > totalPrev && S.view !== 'chat') notify('New message from a friend!', 'info');
        if (S.view === 'friends') { document.getElementById('page').innerHTML = viewFriends(); attachListeners(); }
      }
