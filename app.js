...
const Music = (() => {
  ...
  let playing = false;
  let currentPat = 0;
  let vol = parseFloat(localStorage.getItem('mtcg_vol') || '0.22');
  ...
  function loop(patIdx) {
    ...
    // Melody arpeggio
    seq.forEach((ni, i) => tone(P[ni], now + i * beat, beat * 0.7, 0.058));
    ...
    [P[0], P[2], P[3]].forEach(f => {
      ...
      g.gain.linearRampToValueAtTime(0.022, now + 1.2);
      g.gain.linearRampToValueAtTime(0.022, now + total - 1.2);
    ...
    [0, Math.floor(seq.length/2)].forEach(bi => {
      tone(P[0], now + bi * beat, beat * 2, 0.038, 'triangle');
    });
