...
function adminDeveloper() {
  return `<div class="sketch-box" style="border-color:var(--red)">
    ...
    <h4 style="margin-bottom:0.5rem;color:var(--cyan)">Give Card to Yourself</h4>
    <p class="text-muted mb-1" style="font-size:0.82rem">Commands: <code>/give &lt;card name&gt;</code> &nbsp;|&nbsp; <code>/give id &lt;card id&gt;</code> &nbsp;|&nbsp; or search below.</p>
    <div style="display:flex;gap:0.6rem;margin-bottom:0.5rem">
      <input class="input-box" id="dev-cmd" placeholder="/give Voidwraith  or  /give id 42" style="flex:1;font-family:monospace"
        onkeydown="if(event.key==='Enter')devRunCmd()">
      <button class="btn btn-primary" onclick="devRunCmd()">Run</button>
    </div>
    <div style="display:flex;gap:0.6rem;margin-bottom:0.5rem">
      <input class="input-box" id="dev-card-search" placeholder="Search cards..." style="flex:1"
        onkeydown="if(event.key==='Enter')devCardSearch()">
      <button class="btn btn-sm" onclick="devCardSearch()">Search</button>
    </div>
    <div id="dev-card-results" style="display:flex;flex-wrap:wrap;gap:0.4rem;margin-bottom:0.5rem"></div>
...
window.devRunCmd = async () => {
  const raw = document.getElementById('dev-cmd')?.value?.trim();
  const out = document.getElementById('dev-cmd-out');
  if (!raw || !out) return;
  const setOut = (msg, ok) => out.innerHTML = `<span style="color:${ok ? 'var(--teal)' : 'var(--red)'}">${msg}</span>`;
  if (!raw.startsWith('/')) { setOut('Commands start with /give', false); return; }
  const parts = raw.slice(1).trim().split(/\s+/);
  const cmd = parts[0].toLowerCase();
  if (cmd === 'give') {
    if (parts[1] === 'id' && parts[2]) {
      try { const r = await api('/dev/self/give','POST',{ card_id: parts[2] }); setOut(`✓ ${r.message}`, true); notify(r.message,'success'); } catch(e) { setOut(e.message, false); }
    } else {
      const name = parts.slice(1).join(' ');
      if (!name) { setOut('Usage: /give <card name>', false); return; }
      try { const r = await api('/dev/self/give','POST',{ card_name: name }); setOut(`✓ ${r.message}`, true); notify(r.message,'success'); } catch(e) { setOut(e.message, false); }
    }
  } else {
    setOut(`Unknown command: /${cmd}. Try /give <name>`, false);
  }
};
...
window.devCardSearch = async () => {
  ...
  try {
    ...
    resultsEl.innerHTML = cards.map(c =>
      `<div style="display:flex;align-items:center;gap:0.5rem;background:var(--navy-light);border:1px solid var(--navy-border);border-radius:4px;padding:0.35rem 0.6rem;font-size:0.85rem">
        <span style="color:${typeColor(c.type)}">${c.type}</span>
        <span><strong>${c.name}</strong> <span class="text-muted">#${c.id}</span></span>
        <span class="text-muted" style="font-size:0.75rem">${c.rarity}</span>
        <button class="btn btn-sm btn-primary" style="padding:0.15rem 0.6rem;font-size:0.78rem" onclick="devGiveCard(${c.id},'${c.name.replace(/'/g,"\\'")}')">Give</button>
      </div>`
    ).join('');
  } catch(e) { if (out) out.innerHTML = `<span style="color:var(--red)">${e.message}</span>`; }
};

window.devGiveCard = async (id, name) => {
  const out = document.getElementById('dev-cmd-out');
  try {
    const r = await api('/dev/self/give','POST',{ card_id: id });
...
async function init() {
  ...
  if (S.user) {
    ...
    setInterval(async () => {
      ...
      // Conquest progress
      if (cqProg.value) {
        const changed = JSON.stringify(cqProg.value) !== JSON.stringify(S.conquestProgress);
        S.conquestProgress = cqProg.value;
