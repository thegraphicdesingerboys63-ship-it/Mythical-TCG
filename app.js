...
function renderNav() {
  ...
  const links = [
    ...
    ['trade','Trade'],['friends','Friends'],['coaches','Coaches'],['leaderboard','Leaderboard'],['news','News'],
...
async function openPackModal() {
  ...
  openModal(`<h3 style="margin-bottom:1rem">Opening Pack...</h3>
    ...
    <div id="pack-coach-drop" style="display:none;margin-top:1rem"></div>
  ...
  try {
    ...
    if (data.coachDrop) {
      S.myCoaches.push(data.coachDrop);
      const rarityColors = { Common:'#888', Rare:'#3498db', Epic:'#8b3fc8', Legendary:'#d4a017' };
      const rc = rarityColors[data.coachDrop.rarity] || '#888';
      const el = document.getElementById('pack-coach-drop');
      if (el) {
        el.style.display = 'block';
        el.innerHTML = `<div class="coach-drop-banner" style="border-color:${rc}">
          <span class="coach-drop-portrait">${data.coachDrop.portrait}</span>
          <div>
            <div style="color:${rc};font-weight:700">Coach Obtained!</div>
            <div style="font-weight:600">${data.coachDrop.name} <span class="trait-badge trait-${data.coachDrop.rarity.toLowerCase()}">${data.coachDrop.rarity}</span></div>
            <div class="text-muted" style="font-size:0.85rem">${data.coachDrop.description}</div>
          </div>
        </div>`;
      }
    }
...
function viewCoaches() {
  ...
  const coachCards = S.myCoaches.length
    ? S.myCoaches.map(c => {
        ...
        return `<div class="coach-card${isEq?' coach-card-equipped':''}" style="border-color:${rc}">
          ...
          <div class="coach-card-info">
            ...
            <div class="coach-card-rarity">${c.rarity}</div>
    ...
    : `<p class="text-muted">No coaches yet. Open packs to find them (10% chance per pack)!</p>`;
  return `
  <div class="page-title"><h2>Coaches</h2></div>
  <div class="sketch-box" style="margin-bottom:1rem;padding:1rem">
    <p class="text-muted" style="margin:0">Coaches appear above your card in battle, give buffs, and cheer you on. Found in packs (10% chance). One can be equipped at a time.</p>
  </div>
