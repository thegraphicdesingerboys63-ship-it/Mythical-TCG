...
function renderNav() {
  ...
  const links = [
    ...
    ['trade','Trade'],['friends','Friends'],['leaderboard','Leaderboard'],['news','News'],
...
function _battleCardRow(cards, activeIdx, isPlayer) {
  return cards.map((c,i) => {
    ...
    return `<div class="bcard-mini${active?' bcard-mini-active':''}${!alive?' bcard-mini-fainted':''}"
      onclick="${isPlayer && !active && alive ? `selectBenchCard(${i})` : ''}" title="${c.name}">
      <div class="bcard-mini-art">${generateCardSVG(c)}</div>
      <div class="bcard-mini-name">${c.name.split(' ')[0]}</div>
...
function viewBattle() {
  if (!S.battle || S.battle.finished) {
    ...
    if (S.battle?.finished) {
      ...
      <div class="sketch-box text-center" style="max-width:520px;margin:0 auto;padding:2.5rem">
        ...
        ${result ? `<p style="color:var(--gold);font-weight:700;font-size:1.1rem;margin-bottom:1.2rem">${result.coinsEarned ? '+' + result.coinsEarned + ' coins' : 'No coins earned'}</p>` : ''}
  ...
  const healExhausted = (b.playerHealUses||0) >= 2;
  const combo = b.playerCombo || 0;
  ...
  <div class="battle-arena-v2${b.playerTurn ? ' player-turn-v2' : ''}">
    ...
    <div class="battle-field-v2">
      ...
      <!-- Player active -->
      <div class="battle-active-v2 player-active-v2" id="player-active-slot">
        <div class="bav2-info">
      ...
      ${b.playerTurn && !b.finished ? `
        ...
        <div class="battle-dock-actions">
          ...
          <button class="btn-battle-action btn-ba-heal${(hasOrbsForHeal && !healExhausted)?'':' bba-disabled'}" onclick="battleHeal()" ${(hasOrbsForHeal && !healExhausted)?'':'disabled'}
            ...
            <span class="bba-desc">2 orbs • 25% HP (${2-(b.playerHealUses||0)} left)</span>
          </button>
        </div>
...
// Load trades when navigating to trade view
