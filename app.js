  ...
  <div class="battle-arena">
    ...
    ${(() => {
      ...
      return `<div class="orb-row">
        ${renderOrbMeter(b.playerOrbs?.[pa.type] || 0, cost, `${pa.type}:`)}
        ${renderOrbMeter(b.aiOrbs?.[aa.type] || 0, aiCost, `Foe ${aa.type}:`)}
        ${comboHtml}
      </div>`;
    })()}
...
function renderOrbMeter(orbs, cost, label) {
  const dots = Array.from({length: cost}, (_, i) =>
    `<span class="orb-dot${i < orbs ? ' orb-full' : ''}">◆</span>`
  ).join('');
