...
function _cqSpeak(text) {
  ...
  const trySpeak = () => {
    ...
    const preferred = voices.find(v => /daniel|george|alex|oliver|male|en.gb/i.test(v.name + v.lang));
...
function _cqStartPanel(idx) {
  ...
  // Voice-over
  _cqSpeak((panel.title ? panel.title + '. ' : '') + panel.text);
  // Typewriter
  let i = 0;
