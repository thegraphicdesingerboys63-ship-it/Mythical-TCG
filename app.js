...
function viewConquestBattle() {
  ...
  <div class="battle-arena">
    ...
    ${(() => {
      const cost = clientOrbCost(pa);
      const aiCost = clientOrbCost(aa);
      const combo = b.playerCombo || 0;
      const comboHtml = combo >= 2
        ? `<span class="combo-badge${combo >= 3 ? ' combo-max' : ''}">${combo >= 3 ? '🔥 COMBO x3 — Ability EMPOWERED!' : `Combo x${combo}`}</span>`
        : '';
      return `<div class="orb-row">
        ${renderOrbMeter(b.playerOrbs?.[pa.type] || 0, cost, pa.type, false)}
        ${renderOrbMeter(b.aiOrbs?.[aa.type] || 0, aiCost, aa.type, true)}
        ${comboHtml}
      </div>`;
    })()}
    ${b.bossSurgeActive ? `<div class="boss-surge-warning">⚠️ BOSS ENRAGED — next enemy ability will be devastating!</div>` : ''}
    <div class="battle-controls">
      ${b.playerTurn && !b.finished ? `
        <button class="btn btn-secondary" id="btn-basic" onclick="battleAction('basic')">⚡ Quick Strike</button>
        <button class="btn btn-primary${(b.playerOrbs?.[pa.type]||0) >= clientOrbCost(pa) ? '' : ' orb-locked'}" id="btn-ability" onclick="battleAction('ability')" ${(b.playerOrbs?.[pa.type]||0) >= clientOrbCost(pa) ? '' : 'disabled'}>
          ✦ ${pa.ability_name} <span class="orb-cost-badge">${clientOrbCost(pa)} orbs</span>
        </button>
        <button class="btn btn-secondary" id="btn-guard" onclick="battleAction('guard')">🛡️ Guard</button>
...
function viewBattle() {
  ...
  <div class="battle-arena battle-arena-bg${b.playerTurn?' battle-turn-glow':''}">
    ...
    <div class="battle-controls">
      ${b.playerTurn && !b.finished ? `
        <button class="btn btn-secondary" id="btn-basic" onclick="battleBasic()">⚡ Quick Strike</button>
        <button class="btn btn-primary${(b.playerOrbs?.[pa.type]||0) >= clientOrbCost(pa) ? '' : ' orb-locked'}" id="btn-ability" onclick="battleAbility()" ${(b.playerOrbs?.[pa.type]||0) >= clientOrbCost(pa) ? '' : 'disabled'}>
          ✦ ${pa.ability_name} <span class="orb-cost-badge">${clientOrbCost(pa)} orbs</span>
        </button>
        <button class="btn btn-secondary" id="btn-guard" onclick="battleGuard()">🛡️ Guard</button>
        ${canSwitch ? `<span class="text-muted" style="font-size:0.82rem">click bench to switch</span>` : ''}
...
window.setAdminTab = async (t) => {
  ...
  if (t === 'developer') {
    setTimeout(() => { devLoadMaintenance(); devLoadPacks(); }, 50);
  }
};
  ...
  return `<div class="sketch-box" style="border-color:var(--red)">
    ...
    <div id="maint-status" style="display:flex;gap:0.6rem;flex-wrap:wrap;margin-bottom:0.5rem">
      ${['battle','packs','friends','ranked'].map(f => `
        <div style="display:flex;align-items:center;gap:0.4rem;background:var(--paper-dark);border:1px solid var(--paper-line);border-radius:6px;padding:0.35rem 0.7rem">
          <span style="font-family:var(--font-ui);font-size:0.82rem;text-transform:capitalize">${f}</span>
          <span id="maint-badge-${f}" class="badge badge-green" style="font-size:0.7rem">LIVE</span>
          <button class="btn btn-sm" id="maint-btn-${f}" onclick="devToggleMaint('${f}')" style="padding:0.2rem 0.55rem;font-size:0.78rem">Toggle</button>
        </div>`).join('')}
    </div>
    <button class="btn btn-sm" onclick="devLoadMaintenance()" style="margin-bottom:0.5rem">Refresh Status</button>
    <div id="maint-out" style="font-size:0.85rem;min-height:1rem"></div>
    ...
    <div id="pk-list" style="margin-top:0.5rem"></div>
...
window.devLoadMaintenance = async () => {
  try {
    ...
    for (const f of ['battle','packs','friends','ranked']) {
      ...
      const badge = document.getElementById('maint-badge-' + f);
      const btn   = document.getElementById('maint-btn-' + f);
      if (badge) { badge.textContent = on ? 'MAINT' : 'LIVE'; badge.className = on ? 'badge badge-red' : 'badge badge-green'; }
      if (btn) btn.dataset.state = on ? 'true' : 'false';
    }
    if (out) out.textContent = 'Status refreshed.';
  } catch(e) { notify(e.message,'error'); }
};
window.devToggleMaint = async (feature) => {
  const btn = document.getElementById('maint-btn-' + feature);
  const currentOn = btn?.dataset?.state === 'true';
  const enabled = !currentOn;
  try {
    await api('/dev/maintenance/' + feature, 'PUT', { enabled });
    const badge = document.getElementById('maint-badge-' + feature);
    if (badge) { badge.textContent = enabled ? 'MAINT' : 'LIVE'; badge.className = enabled ? 'badge badge-red' : 'badge badge-green'; }
    if (btn) btn.dataset.state = String(enabled);
    notify(`${feature} maintenance ${enabled ? 'ENABLED' : 'DISABLED'}`, enabled ? 'error' : 'success');
  } catch(e) { notify(e.message,'error'); }
};
window.devCreatePack = async () => {
  ...
  try {
    ...
    document.getElementById('pk-out').textContent = '✓ Created';
    devLoadPacks();
  } catch(e) { notify(e.message,'error'); }
};
window.devLoadPacks = async () => {
  try {
    const packs = await api('/dev/packs');
    const el = document.getElementById('pk-list');
    if (!el) return;
    if (!packs.length) { el.innerHTML = '<p class="text-muted" style="font-size:0.85rem">No custom packs yet.</p>'; return; }
...
window.devDeletePack = async (packId) => {
  ...
  try { await api('/dev/packs/' + packId, 'DELETE'); notify('Pack deleted', 'success'); devLoadPacks(); }
  catch(e) { notify(e.message,'error'); }
};
