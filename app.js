...
function viewQuests() {
  ...
  const renderQuest = q => {
    ...
    const barColor = q.completed ? '#2ecc71' : '#4dd9ff';
    const statusClass = q.claimed ? 'quest-claimed' : q.completed ? 'quest-complete' : '';
    return `
    <div class="quest-card ${statusClass}">
      <div class="quest-icon">${q.icon}</div>
      <div class="quest-info">
        <div class="quest-name">${q.name}</div>
        <div class="quest-desc text-muted">${q.description}</div>
        <div class="quest-progress-wrap">
          <div class="quest-progress-bar-outer">
            <div class="quest-progress-bar" style="width:${pct}%;background:${barColor}"></div>
          </div>
          <span class="quest-progress-text">${q.progress}/${q.target}</span>
        </div>
      </div>
      <div class="quest-reward">
        <span class="quest-xp-badge">+${q.xp_reward} XP</span>
        ${q.claimed
          ? `<span class="quest-done-badge">✓ Done</span>`
          : q.completed
            ? `<button class="btn btn-primary btn-sm" onclick="claimQuest(${q.id})">Claim</button>`
            : ``}
      </div>
    </div>`;
  };
  ...
  const nextReward = S.bpRewards.find(r => r.level === bpLevel + 1);
  const xpToNext = nextReward ? nextReward.xp_required - bpXp : null;
  const bpPct = nextReward ? Math.min(100, Math.round(((bpXp - (S.bpRewards.find(r=>r.level===bpLevel)?.xp_required||0)) / (nextReward.xp_required - (S.bpRewards.find(r=>r.level===bpLevel)?.xp_required||0))) * 100)) : 100;

  return `
  <div class="page-title"><h2>Quests</h2><p class="text-muted">Complete quests to earn XP for the Battle Pass</p></div>

  <div class="quest-bp-summary sketch-box">
    <div class="qbp-left">
      <div class="qbp-level-badge">Lv.${bpLevel}</div>
      <div class="qbp-info">
        <div class="qbp-title">Battle Pass Progress</div>
        <div class="qbp-xp-row">
          <div class="qbp-bar-outer"><div class="qbp-bar" style="width:${bpPct}%"></div></div>
          <span class="qbp-xp-text">${bpXp} XP${xpToNext !== null ? ` · ${xpToNext} to next level` : ' · Max level!'}</span>
        </div>
      </div>
    </div>
    <button class="btn btn-gold" onclick="nav('battlepass')">View Pass →</button>
  </div>

  <div class="quest-section">
    <h3 class="quest-section-title">📅 Daily Quests <span class="text-muted" style="font-size:0.8rem">Resets at midnight</span></h3>
    ${daily.length ? daily.map(renderQuest).join('') : '<p class="text-muted" style="padding:0.5rem 0">No daily quests.</p>'}
  </div>

  <div class="quest-section" style="margin-top:1.5rem">
    <h3 class="quest-section-title">📆 Weekly Quests <span class="text-muted" style="font-size:0.8rem">Resets weekly</span></h3>
    ${weekly.length ? weekly.map(renderQuest).join('') : '<p class="text-muted" style="padding:0.5rem 0">No weekly quests.</p>'}
  </div>`;
}
...
function viewBattlepass() {
  ...
  const rewardCards = rewards.map(r => {
    ...
    return `
    <div class="bp-reward-card ${unlocked ? 'bp-unlocked' : 'bp-locked'} ${claimed ? 'bp-claimed' : ''} ${canClaim ? 'bp-claimable' : ''}">
      <div class="bp-reward-level">Lv.${r.level}</div>
      <div class="bp-reward-icon">${r.reward_icon}</div>
      <div class="bp-reward-label">${r.reward_label}</div>
      ${claimed
        ? `<div class="bp-claimed-badge">✓</div>`
        : canClaim
          ? `<button class="btn btn-primary btn-sm bp-claim-btn" onclick="claimBpReward(${r.level})">Claim</button>`
          : !unlocked
            ? `<div class="bp-locked-badge">🔒 ${r.xp_required} XP</div>`
            : ``}
    </div>`;
  }).join('');

  return `
  <div class="page-title"><h2>⚔️ Battle Pass — Season 1</h2><p class="text-muted">Earn XP from quests to unlock rewards</p></div>

  <div class="bp-header sketch-box">
    <div class="bp-level-display">
      <span class="bp-level-num">Level ${currentLevel}</span>
      <span class="text-muted" style="font-size:0.85rem">/ 30</span>
    </div>
    <div class="bp-xp-section">
      <div class="bp-xp-bar-outer">
        <div class="bp-xp-bar" style="width:${overallPct}%"></div>
      </div>
      <div class="bp-xp-label">${currentXp.toLocaleString()} / ${totalXpForMax.toLocaleString()} XP${nextReward ? ` · ${nextReward.xp_required - currentXp} XP to Lv.${nextReward.level}` : ' · Max!'}</div>
    </div>
    <button class="btn btn-sm" onclick="nav('quests')">📋 Do Quests</button>
  </div>

  <div class="bp-rewards-grid">${rewardCards}</div>`;
}
