...
// ─── MUSIC SYSTEM ─────────────────────────────────────────────────
// ─── PIANO (procedural soft piano via Web Audio) ──────────────────
const PianoMusic = (() => {
  let ctx = null, playing = false, nodes = [], schedulerTimer = null;
  // C major pentatonic, two octaves
  const NOTES = [261.63,293.66,329.63,392,440,523.25,587.33,659.25,783.99,880];
  const SEQ   = [0,2,4,5,7,4,2,0,5,7,9,7,5,2,4,0]; // indices into NOTES
  let beat = 0;
  ...
  function _scheduleNote(freq, when, dur, vol) {
  ...
  function _tick(masterVol) {
    if (!playing) return;
    const now = ctx.currentTime;
    const interval = 0.55; // seconds per beat (≈109 BPM feel)
    // Schedule 4 beats ahead
    for (let i = 0; i < 4; i++) {
      const when = now + i * interval;
      const noteIdx = SEQ[(beat + i) % SEQ.length];
      const freq = NOTES[noteIdx];
      _scheduleNote(freq, when, interval * 0.85, masterVol * 0.45);
      // Occasional soft octave up
      if ((beat + i) % 4 === 0) _scheduleNote(freq * 2, when + 0.01, interval * 0.5, masterVol * 0.18);
    }
    beat = (beat + 4) % SEQ.length;
    schedulerTimer = setTimeout(() => _tick(masterVol), 4 * interval * 1000 - 80);
  }
  return {
    start(vol) { _boot(); playing = true; beat = 0; _tick(vol); },
    stop() {
      playing = false;
...
const Music = (() => {
  ...
  // 'piano' or 'soundtrack'
  let track = localStorage.getItem('mtcg_main_track') || 'piano';
  ...
  return {
    ...
    setTrack(t) {
      track = t;
      localStorage.setItem('mtcg_main_track', t);
      if (playing) { audio.pause(); PianoMusic.stop(); _startTrack(); }
    },
    autoStart() {
...
function getView() {
  switch (S.view) {
    ...
    case 'crafting':      return viewCrafting();
...
function renderNav() {
  ...
  const links = [
    ...
    ['trade','Trade'],['friends','Friends'],['coaches','Coaches'],['quests','Quests'],['battlepass','Battle Pass'],['achievements','Achievements'],['crafting','Forge'],['leaderboard','Leaderboard'],['news','News'],
...
// ─── HOME ─────────────────────────────────────────────────────────
// ─── TUTORIAL ─────────────────────────────────────────────────────
// Each step: title, body, target (CSS selector), action (optional {label, fn})
// clickAdvance: clicking the highlighted element also advances
const TUTORIAL_STEPS = [
  {
    title: 'Welcome to Mythical TCG! 👋',
    body: "You're about to learn how to play. Each step highlights something — click it or press Next. You can skip at any time.",
    target: null,
    icon: '🃏',
  },
  {
    title: '🛒 The Shop',
    body: 'Packs cost 100 coins and give you 5 random cards. You start with 200 coins — try opening one now!',
    target: `[onclick="nav('shop')"]`,
    action: { label: 'Go to Shop', fn: () => { nav('shop'); tutNav(_tutStep + 1); } },
    clickAdvance: true,
  },
  {
    title: '🃏 Your Collection',
    body: 'Every card you own lives here. Click a card to see its full stats, flavor text, and traits.',
    target: `[onclick="nav('collection')"]`,
    action: { label: 'View Collection', fn: () => { nav('collection'); tutNav(_tutStep + 1); } },
    clickAdvance: true,
  },
  {
    title: '🗂️ Build Your Deck',
    body: 'Pick up to 10 cards from your collection. Your deck is used in every battle — a strong deck wins matches!',
    target: `[onclick="nav('deck')"]`,
    action: { label: 'Open Deck Builder', fn: () => { nav('deck'); tutNav(_tutStep + 1); } },
    clickAdvance: true,
  },
  {
    title: '⚔️ How Battles Work',
    body: '<strong>Each turn:</strong><br>1. Attach one energy to a card<br>2. Choose an action: Quick Strike, Ability, Guard, Boost, or Heal<br><br>Cards faint when HP hits 0. Last team standing wins!',
    target: `[onclick="nav('battle')"]`,
    action: { label: 'Start a Battle', fn: () => { nav('battle'); tutNav(_tutStep + 1); } },
    clickAdvance: true,
  },
  {
    title: '🗺️ Conquest Mode',
    body: 'A story campaign with chapters and boss stages. Beating stages earns <strong>Traits</strong> — permanent stat buffs you equip to your cards.',
    target: `[onclick="nav('conquest')"]`,
    action: { label: 'View Conquest', fn: () => { nav('conquest'); tutNav(_tutStep + 1); } },
    clickAdvance: true,
  },
  {
    title: '🏆 PvP — Live Battles',
    body: 'Fight real players! Win to earn <strong>rating</strong> and climb the ranked leaderboard. You can even chat with your opponent mid-match.',
    target: `[onclick="nav('pvp')"]`,
    action: { label: 'View PvP', fn: () => { nav('pvp'); tutNav(_tutStep + 1); } },
    clickAdvance: true,
  },
  {
    title: '🎯 Quests & Battle Pass',
    body: 'Daily and weekly quests reward <strong>XP</strong>. XP levels up your Battle Pass — unlocking coins and card packs along the way.',
    target: `[onclick="nav('quests')"]`,
    action: { label: 'View Quests', fn: () => { nav('quests'); tutNav(_tutStep + 1); } },
    clickAdvance: true,
  },
  {
    title: "You're ready! 🎉",
    body: "That's everything you need to know. Claim your starter reward below and start your journey!",
    target: null,
    icon: '🏆',
    finish: true,
  },
...
function _renderTutStep() {
  // Remove old overlay + any lingering click handler
  document.getElementById('tutorial-overlay')?.remove();
  if (_tutClickHandler) { document.removeEventListener('click', _tutClickHandler, true); _tutClickHandler = null; }
  ...
  if (target) {
    ...
    // Pulse arrow pointing to target
    const arrow = document.createElement('div');
    arrow.className = 'tut-arrow';
    arrow.style.cssText = `top:${r.bottom + pad + 2}px;left:${r.left + r.width / 2}px`;
    ...
    const tipTop = r.bottom + pad + 22;
    tip.style.cssText = `left:${tipLeft}px;top:${Math.min(tipTop, window.innerHeight - 230)}px`;
    overlay.appendChild(tip);

    // Clicking the highlighted element advances (if clickAdvance)
    if (step.clickAdvance) {
      _tutClickHandler = (e) => {
        if (target.contains(e.target) || e.target === target) {
          e.stopPropagation();
          document.removeEventListener('click', _tutClickHandler, true);
          _tutClickHandler = null;
          tutNav(_tutStep + 1);
        }
      };
      document.addEventListener('click', _tutClickHandler, true);
    }
  } else {
  ...
  requestAnimationFrame(() => {
    ...
    if (t) { t.style.opacity = '0'; t.style.transform = 'translateY(8px) scale(0.97)'; requestAnimationFrame(() => { t.style.transition = 'opacity 0.2s,transform 0.2s'; t.style.opacity = '1'; t.style.transform = 'translateY(0) scale(1)'; }); }
  });
}
...
function _buildTip(step, isFirst, isLast) {
  ...
  const dots = TUTORIAL_STEPS.map((_, i) =>
    `<span class="tut-dot${i === _tutStep ? ' tut-dot-active' : i < _tutStep ? ' tut-dot-done' : ''}"></span>`).join('');

  const actionBtn = step.action
    ? `<button class="btn btn-gold btn-sm tut-action-btn" onclick="(${step.action.fn.toString()})()">${step.action.label} ↗</button>`
    : '';

  const icon = step.icon ? `<div class="tut-icon">${step.icon}</div>` : '';
  ...
  tip.innerHTML = `
    ...
    <div class="tut-btns">
      <button class="btn btn-sm tut-skip" onclick="skipTutorial()">Skip tutorial</button>
      <div style="display:flex;gap:0.4rem;align-items:center">
        ${!isFirst ? `<button class="btn btn-sm" onclick="tutNav(${_tutStep - 1})">← Back</button>` : ''}
        ${isLast
          ? `<button class="btn btn-primary btn-sm tut-finish-btn" onclick="finishTutorial()">Claim reward 🎁</button>`
          : step.clickAdvance
            ? `<span class="tut-hint">or click the highlighted item</span>`
...
function tutNav(n) {
  if (_tutClickHandler) { document.removeEventListener('click', _tutClickHandler, true); _tutClickHandler = null; }
  _tutStep = Math.max(0, Math.min(n, TUTORIAL_STEPS.length - 1));
  _renderTutStep();
}
  ...
  try {
    ...
    openModal(`
      <div class="text-center">
        ...
        <h3 style="margin-bottom:0.4rem">Tutorial Complete!</h3>
        <p class="text-muted mb-2">Here's your starter reward:</p>
        <div style="display:flex;justify-content:center;gap:2rem;margin-bottom:1.2rem">
          <div style="text-align:center">
            <div style="font-size:2rem">🪙</div>
            <div style="font-size:1.4rem;font-weight:800;color:var(--gold-light)">+150</div>
            <div style="font-size:0.75rem;color:var(--ink-light)">Coins</div>
          </div>
          <div style="text-align:center">
            <div style="font-size:2rem">🃏</div>
            <div style="font-size:1.4rem;font-weight:800;color:var(--cyan-bright)">+3</div>
...
function viewPvp() {
  ...
  <div style="max-width:520px;margin:0 auto">
    ...
    <div class="sketch-box mb-2">
      <h3 style="margin-bottom:0.5rem">🔍 Scout an Opponent</h3>
      <p class="text-muted mb-1" style="font-size:0.85rem">Spend 50 coins to preview a player's top 3 cards before challenging them.</p>
      <div style="display:flex;gap:0.6rem;flex-wrap:wrap">
        <input class="input-box" id="scout-username" placeholder="Enter username..." style="flex:1">
        <button class="btn btn-primary" onclick="scoutPlayer()">Scout (50 🪙)</button>
      </div>
      ${S.scoutData ? `<div class="scout-results mt-1">
        <p style="font-size:0.82rem;color:var(--ink-light);margin-bottom:0.4rem">Top 3 cards:</p>
        ${S.scoutData.map(c => `<div class="scout-card-row">
          <span class="scout-type" style="color:var(--c-${c.type.toLowerCase()},#888)">${c.type}</span>
          <strong>${c.name}</strong>
          <span class="text-muted" style="font-size:0.78rem">ATK:${c.atk} HP:${c.hp}</span>
          <span class="rarity-label rarity-${c.rarity?.toLowerCase()?.replace(' ','_')}">${c.rarity}</span>
        </div>`).join('')}
      </div>` : ''}
    </div>
    <div class="sketch-box">
...
window.scoutPlayer = async () => {
  const u = document.getElementById('scout-username')?.value?.trim();
  if (!u) return;
  try {
    const r = await api('/pvp/scout', 'POST', { targetUsername: u });
    S.scoutData = r.cards;
    S.user.coins -= 50;
    updateNavCoins();
    document.getElementById('page').innerHTML = viewPvp();
    attachListeners();
  } catch(e) { notify(e.message, 'error'); }
};
...
function viewPvpBattle() {
  ...
  function weatherBadgePvp(weather) {
    if (!weather) return '';
    const info = {
      storm:   { icon:'⛈️', label:'Storm: +15% Thunder', cls:'badge-storm' },
      drought: { icon:'☀️', label:'Drought: +15% Fire/-10% Water', cls:'badge-drought' },
      fog:     { icon:'🌫️', label:'Fog: -10% all attacks', cls:'badge-fog' },
    };
    const w = info[weather];
    return w ? `<span class="weather-badge ${w.cls}">${w.icon} ${w.label}</span>` : '';
  }
  ...
  <div class="battle-arena-v2${b.playerTurn ? ' player-turn-v2' : ''}">
    ...
    <!-- Weather -->
    ${b.weather ? `<div style="text-align:center;margin-bottom:0.3rem">${weatherBadgePvp(b.weather)}</div>` : ''}
...
function showCardDetail(id) {
  ...
  const traitSection = equippedTrait
    ...
    : (isOwned && S.myTraits.length
        ...
        : '');
  const giftBtn = isOwned ? `<button class="btn btn-sm" style="margin-top:0.4rem" onclick="closeModal();openGiftModal(${id},'${card.name.replace(/'/g,"\\'")}')">🎁 Gift Card</button>` : '';
...
function openGiftModal(cardId, cardName) {
  openModal(`<div>
    <h3 style="margin-bottom:0.8rem">🎁 Gift ${escHtml(cardName)}</h3>
    <p class="text-muted mb-2" style="font-size:0.85rem">Send this card to a friend. You can gift once per friend per day.</p>
    <div class="form-group"><label>Friend's Username</label><input id="gift-recipient" class="input-box" placeholder="Username"></div>
    <div style="display:flex;gap:0.6rem;margin-top:0.8rem">
      <button class="btn btn-primary" onclick="submitGift(${cardId})">Send Gift</button>
      <button class="btn" onclick="closeModal()">Cancel</button>
    </div>
  </div>`);
}
...
window.submitGift = async (cardId) => {
  const recipient = document.getElementById('gift-recipient')?.value?.trim();
  if (!recipient) return notify('Enter a username', 'error');
  try {
    const r = await api(`/cards/${cardId}/gift`, 'POST', { recipientUsername: recipient });
    notify(r.message, 'success');
    if (r.newAchievements?.length) showAchievementPopup(r.newAchievements);
    closeModal();
    S.collection = await api('/user/collection');
...
function viewBattle() {
  ...
  <div class="battle-arena-v2${b.playerTurn ? ' player-turn-v2' : ''}">
    ...
    <!-- Weather -->
    ${b.weather ? `<div style="text-align:center;margin-bottom:0.3rem">${weatherBadge(b.weather)}</div>` : ''}
...
window.nav = function(view) {
  ...
  if (view === 'crafting') { document.getElementById('page').innerHTML = viewCrafting(); attachListeners(); }
};
...
async function loadAchievements() {
  try {
    ...
    S.allAchievements = data.all || [];
    S.myAchievements  = data.unlocked || [];
    S._achUnlockedAt  = data.unlockedAt || {};
    document.getElementById('page').innerHTML = viewAchievements();
    attachListeners();
  } catch(e) { notify(e.message, 'error'); }
}

function viewAchievements() {
  const cats = [...new Set((S.allAchievements||[]).map(a => a.category))];
  const unlockedCount = S.myAchievements.length;
  const totalCount = S.allAchievements.length;
  const sections = cats.map(cat => {
    const achs = S.allAchievements.filter(a => a.category === cat);
    const rows = achs.map(a => {
      const done = S.myAchievements.includes(a.id);
      const when = done && S._achUnlockedAt?.[a.id] ? `<span class="ach-date">${new Date(S._achUnlockedAt[a.id]).toLocaleDateString()}</span>` : '';
      return `<div class="ach-card${done ? ' ach-unlocked' : ' ach-locked'}">
        <span class="ach-icon">${a.icon}</span>
        <div class="ach-info">
          <div class="ach-name">${a.name}</div>
          <div class="ach-desc">${a.description}</div>
          ${a.reward_coins ? `<div class="ach-reward">+${a.reward_coins} coins ${done ? '(claimed)' : '(on unlock)'}</div>` : ''}
          ${when}
        </div>
    ...
    return `<div class="ach-section"><h3 class="ach-cat-header">${catLabel}</h3>${rows}</div>`;
  }).join('');
  return `<div class="page-title"><h2>Achievements</h2>
    <p class="text-muted">${unlockedCount} / ${totalCount} unlocked</p></div>
    <div class="ach-progress-bar-wrap mb-2">
      <div class="ach-progress-bar" style="width:${totalCount ? Math.round(unlockedCount/totalCount*100) : 0}%"></div>
    </div>
    <div class="ach-grid">${sections}</div>`;
}

function showAchievementPopup(achievements) {
  if (!achievements?.length) return;
  const html = achievements.map(a => `
    <div class="ach-popup-item">
      <span class="ach-popup-icon">${a.icon}</span>
      <div><strong>${a.name}</strong><br><span class="text-muted" style="font-size:0.82rem">${a.description}</span>
      ${a.reward_coins ? `<br><span style="color:var(--gold-light)">+${a.reward_coins} coins</span>` : ''}</div>
    </div>`).join('');
  const el = document.createElement('div');
  el.className = 'ach-toast-container';
  el.innerHTML = `<div class="ach-toast"><div class="ach-toast-title">🏆 Achievement Unlocked!</div>${html}</div>`;
  document.body.appendChild(el);
...
function viewCrafting() {
  const rareCards = S.collection.filter(c => c.rarity === 'Rare' && c.quantity >= 1);
  const selected = S.craftSelected || [];
  const byType = {};
  rareCards.forEach(c => { if (!byType[c.type]) byType[c.type] = []; byType[c.type].push(c); });

  const canForge = selected.length === 5;
  const selSet = new Set(selected);

  const typeGroups = Object.entries(byType).map(([type, cards]) => {
    const rows = cards.map(c => {
      const isSel = selSet.has(c.id);
      return `<div class="craft-card-row${isSel ? ' craft-selected' : ''}" onclick="toggleCraftSelect(${c.id})">
        <span class="craft-type-dot" style="background:var(--c-${type.toLowerCase()},#888)"></span>
        <span class="craft-card-name">${c.name}</span>
        <span class="text-muted" style="font-size:0.78rem">×${c.quantity}</span>
        ${isSel ? '<span style="color:var(--gold-light)">✓</span>' : ''}
      </div>`;
    }).join('');
    return `<div class="craft-type-group"><div class="craft-type-header">${type} (${cards.length})</div>${rows}</div>`;
  }).join('');

  return `<div class="page-title"><h2>🔨 Mythic Forge</h2></div>
    <div class="craft-layout">
      <div class="sketch-box">
        <h3 style="margin-bottom:0.5rem">Select 5 Rare cards of the same type</h3>
        <p class="text-muted mb-2" style="font-size:0.85rem">Consuming 5 Rare cards of the same type will forge a random Legendary of that type.</p>
        <div class="craft-selection-bar">
          ${selected.map(id => { const c = S.collection.find(x => x.id===id); return `<div class="craft-sel-chip">${c?.name||'?'} <span onclick="toggleCraftSelect(${id})">✕</span></div>`; }).join('')}
          ${Array(5-selected.length).fill(0).map(()=>'<div class="craft-sel-chip craft-sel-empty">—</div>').join('')}
        </div>
        <button class="btn btn-primary mt-1${canForge ? '' : ' btn-disabled'}" onclick="submitForge()" ${canForge ? '' : 'disabled'}>
          ${canForge ? '🔥 Forge Legendary' : `Select ${5-selected.length} more`}
        </button>
      </div>
      <div class="sketch-box mt-2">
        <h3 style="margin-bottom:0.8rem">Your Rare Cards</h3>
        <div class="craft-card-list">${typeGroups || '<p class="text-muted">No Rare cards in your collection.</p>'}</div>
      </div>
    </div>`;
}

window.toggleCraftSelect = (id) => {
  const sel = S.craftSelected || [];
  const idx = sel.indexOf(id);
  if (idx !== -1) { S.craftSelected = sel.filter(x => x !== id); }
  else if (sel.length < 5) { S.craftSelected = [...sel, id]; }
  document.getElementById('page').innerHTML = viewCrafting();
  attachListeners();
};

window.submitForge = async () => {
  const cardIds = S.craftSelected || [];
  if (cardIds.length !== 5) return;
  try {
    const r = await api('/crafting/mythic-forge', 'POST', { cardIds });
    S.craftSelected = [];
    S.collection = await api('/user/collection');
    if (r.newAchievements?.length) showAchievementPopup(r.newAchievements);
    openModal(`<div class="text-center">
      <h3>⚡ Legendary Forged!</h3>
      <p class="text-muted mb-2">Your sacrifice was accepted.</p>
      <div style="display:flex;justify-content:center">${renderCard(r.card)}</div>
      <button class="btn btn-primary mt-2" onclick="closeModal();nav('crafting')">Continue</button>
    </div>`);
  } catch(e) { notify(e.message, 'error'); }
};
...
function viewSettings() {
  ...
  const sections = {
    ...
    appearance: `
      ...
      <div class="form-group">
        <label>Main Music Track</label>
        <div style="display:flex;gap:0.6rem;flex-wrap:wrap">
          <button class="btn btn-sm${Music.track==='piano'?' btn-primary':''}" id="track-piano" onclick="Music.setTrack('piano');document.getElementById('track-piano').className='btn btn-sm btn-primary';document.getElementById('track-soundtrack').className='btn btn-sm';">🎹 Soft Piano</button>
          <button class="btn btn-sm${Music.track==='soundtrack'?' btn-primary':''}" id="track-soundtrack" onclick="Music.setTrack('soundtrack');document.getElementById('track-soundtrack').className='btn btn-sm btn-primary';document.getElementById('track-piano').className='btn btn-sm';">🎵 Soundtrack</button>
        </div>
      </div>
      <div class="form-group">
...
function viewAdmin() {
  ...
  const tabs = [
    ...
    ['stats','Stats',2],['cards','Cards',3],['economy','Economy',3],['battles','Battles',3],
...
function renderAdminTab() {
  switch(S.adminTab) {
    ...
    case 'battles':   return adminBattles();
...
async function loadAdminTabData(t) {
  try {
    ...
    if (t === 'economy') {
      api('/admin/economy/analytics').then(d => { S._adminEconomy = d; document.getElementById('admin-content').innerHTML = adminEconomy(); }).catch(()=>{});
    }
    if (t === 'battles') {
      api('/admin/battles/active').then(d => { S._adminBattles = d; document.getElementById('admin-content').innerHTML = adminBattles(); }).catch(()=>{});
    }
    if (t === 'developer') {
...
function adminLogs() {
  ...
  const rows = logs.map(l => `<tr>
    <td>${l.id}</td>
    <td>${l.admin_name||'?'}</td>
    <td style="font-size:0.8rem">${l.action}</td>
    <td>${l.target_user_id||'-'}</td>
    <td style="max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${(l.details||'').replace(/"/g,'')}">${(l.details||'').slice(0,80)}</td>
    <td style="font-size:0.78rem;white-space:nowrap">${new Date(l.created_at).toLocaleString()}</td>
  </tr>`).join('') || '<tr><td colspan="6" class="text-muted text-center">No logs</td></tr>';
  return `<div class="flex gap-1 mb-1" style="flex-wrap:wrap">
    <input id="log-action" class="input-box" placeholder="Filter action..." style="flex:1;min-width:130px">
    <input id="log-admin" class="input-box" placeholder="Admin username..." style="flex:1;min-width:130px">
    <input id="log-from" class="input-box" type="date" style="width:140px">
    <input id="log-to" class="input-box" type="date" style="width:140px">
    <button class="btn btn-sm" onclick="adminFilterLogs()">Filter</button>
    <button class="btn btn-sm" onclick="adminClearLogFilter()">Clear</button>
  </div>
  <div style="overflow-x:auto"><table class="admin-table">
...
window.adminFilterLogs = async () => {
  const params = new URLSearchParams();
  const action = document.getElementById('log-action')?.value; if (action) params.set('action', action);
  const admin  = document.getElementById('log-admin')?.value;  if (admin)  params.set('adminName', admin);
  const from   = document.getElementById('log-from')?.value;   if (from)   params.set('from', from);
  const to     = document.getElementById('log-to')?.value;     if (to)     params.set('to', to);
  try {
    S._adminLogs = await api('/admin/logs/filtered?' + params.toString());
    document.getElementById('admin-content').innerHTML = adminLogs();
  } catch(e) { notify(e.message,'error'); }
};
window.adminClearLogFilter = async () => {
  try { S._adminLogs = await api('/admin/logs'); document.getElementById('admin-content').innerHTML = adminLogs(); } catch(e) { notify(e.message,'error'); }
};
...
function adminEconomy() {
  const eco = S._adminEconomy || {};
  const c = eco.coins || {};
  const t = eco.trades || {};
  const u = eco.users || {};
  const daily = eco.dailySignups || [];

  const sparkline = daily.length ? daily.slice(0,7).reverse().map(d =>
    `<div style="display:flex;align-items:flex-end;gap:2px">
      <div style="width:28px;height:${Math.max(4, Math.round((d.cnt/Math.max(...daily.map(x=>x.cnt)))*40))}px;background:var(--cyan-dark);border-radius:2px 2px 0 0" title="${d.day}: ${d.cnt}"></div>
    </div>`).join('') : '';

  return `<div class="grid-4 gap-1 mb-2">
    <div class="sketch-box text-center"><h3 style="color:var(--gold-light)">${(parseInt(c.total)||0).toLocaleString()}</h3><p class="text-muted">Coins in Circulation</p></div>
    <div class="sketch-box text-center"><h3>${(parseInt(c.avg)||0).toLocaleString()}</h3><p class="text-muted">Avg Coins/User</p></div>
    <div class="sketch-box text-center"><h3>${(parseInt(t.total)||0).toLocaleString()}</h3><p class="text-muted">Total Trades</p></div>
    <div class="sketch-box text-center"><h3>${(parseInt(eco.totalMatches)||0).toLocaleString()}</h3><p class="text-muted">Total Battles</p></div>
  </div>
  <div class="grid-2 gap-2 mb-2">
    <div class="sketch-box">
      <h4 style="margin-bottom:0.6rem">Coin Distribution</h4>
      <div class="stat-row"><span class="label">Max Coins</span><span>${(parseInt(c.max)||0).toLocaleString()}</span></div>
      <div class="stat-row"><span class="label">Min Coins</span><span>${(parseInt(c.min)||0).toLocaleString()}</span></div>
      <div class="stat-row"><span class="label">Trade Completion</span><span>${t.total ? Math.round(t.completed/t.total*100) : 0}%</span></div>
    </div>
    <div class="sketch-box">
      <h4 style="margin-bottom:0.6rem">New Users (7 days)</h4>
      <div style="display:flex;align-items:flex-end;gap:3px;height:44px">${sparkline}</div>
      <p class="text-muted" style="font-size:0.78rem;margin-top:0.3rem">${(parseInt(u.new_week)||0)} new this week / ${(parseInt(u.total)||0)} total</p>
    </div>
  </div>
  <div class="sketch-box mb-2">
    <h3 style="margin-bottom:1rem">Give Coins to User</h3>
    <div class="flex gap-1">
      <input id="eco-uid" class="input-box" placeholder="User ID" type="number" style="width:120px">
      <input id="eco-amt" class="input-box" placeholder="Amount (negative to remove)" type="number" style="flex:1">
      <button class="btn btn-primary" onclick="adminGiveCoins()" style="white-space:nowrap">Apply</button>
    </div>
  </div>
  <div class="sketch-box mb-2">
    <h3 style="margin-bottom:0.8rem">Bulk Coin Grant</h3>
    <p class="text-muted mb-1" style="font-size:0.85rem">Apply coins to multiple user IDs (comma separated)</p>
    <div class="flex gap-1 mb-1">
      <input id="bulk-coin-ids" class="input-box" placeholder="1, 2, 3..." style="flex:2">
      <input id="bulk-coin-amt" class="input-box" type="number" placeholder="Amount" style="width:110px">
      <button class="btn btn-primary" onclick="adminBulkCoins()" style="white-space:nowrap">Apply</button>
    </div>
  </div>
  <div class="sketch-box">
...
window.adminBulkCoins = async () => {
  const ids = (document.getElementById('bulk-coin-ids')?.value||'').split(',').map(s=>parseInt(s.trim())).filter(n=>!isNaN(n));
  const amt = parseInt(document.getElementById('bulk-coin-amt')?.value);
  if (!ids.length || isNaN(amt)) { notify('Enter user IDs and amount','error'); return; }
  try { await api('/admin/users/bulk-coins','POST',{userIds:ids,amount:amt}); notify(`Applied ${amt} coins to ${ids.length} users`,'success'); } catch(e) { notify(e.message,'error'); }
};

function adminBattles() {
  const data = S._adminBattles || { ai: [], pvp: [], queueSize: 0 };
  const wIcons = { storm:'⛈️', drought:'☀️', fog:'🌫️' };
  const aiRows = data.ai.map(b => `<tr>
    <td>${b.userId}</td>
    <td>${b.turns} turns</td>
    <td>${b.elapsed}s</td>
    <td>${wIcons[b.weather]||'—'} ${b.weather||'none'}</td>
    <td>${b.finished ? '✅ Done' : '⚔️ Active'}</td>
    <td><button class="btn btn-sm btn-red" onclick="adminForceEndAI(${b.userId})">End</button></td>
  </tr>`).join('') || '<tr><td colspan="6" class="text-muted text-center">No active AI battles</td></tr>';
  const pvpRows = data.pvp.map(b => `<tr>
    <td style="font-size:0.78rem">${b.id.slice(0,20)}...</td>
    <td>${b.p1} vs ${b.p2}</td>
    <td>${b.elapsed}s</td>
    <td>${b.ranked ? '🏆 Ranked' : 'Casual'}</td>
    <td>${wIcons[b.weather]||'—'} ${b.weather||'none'}</td>
    <td>${b.finished ? '✅ Done' : '⚔️ Active'}</td>
    <td><button class="btn btn-sm btn-red" onclick="adminForceEndPvP('${b.id}')">End</button></td>
  </tr>`).join('') || '<tr><td colspan="7" class="text-muted text-center">No active PvP battles</td></tr>';
  return `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.8rem">
    <h3>Live Battles</h3>
    <div style="display:flex;gap:0.6rem;align-items:center">
      <span class="text-muted" style="font-size:0.85rem">Queue: ${data.queueSize}</span>
      <button class="btn btn-sm" onclick="adminRefreshBattles()">Refresh</button>
    </div>
  </div>
  <h4 class="mb-1">AI Battles (${data.ai.length})</h4>
  <div style="overflow-x:auto;margin-bottom:1rem"><table class="admin-table">
    <thead><tr><th>User</th><th>Turns</th><th>Elapsed</th><th>Weather</th><th>Status</th><th>Action</th></tr></thead>
    <tbody>${aiRows}</tbody>
  </table></div>
  <h4 class="mb-1">PvP Battles (${data.pvp.length})</h4>
  <div style="overflow-x:auto"><table class="admin-table">
    <thead><tr><th>ID</th><th>Players</th><th>Elapsed</th><th>Mode</th><th>Weather</th><th>Status</th><th>Action</th></tr></thead>
    <tbody>${pvpRows}</tbody>
  </table></div>`;
}
window.adminRefreshBattles = async () => {
  try { S._adminBattles = await api('/admin/battles/active'); document.getElementById('admin-content').innerHTML = adminBattles(); } catch(e) { notify(e.message,'error'); }
};
window.adminForceEndAI = async (uid) => {
  try { await api(`/dev/battles/ai/${uid}`,'DELETE'); notify('AI battle ended','success'); adminRefreshBattles(); } catch(e) { notify(e.message,'error'); }
};
window.adminForceEndPvP = async (id) => {
  try { await api(`/dev/battles/pvp/${id}`,'DELETE'); notify('PvP battle ended','success'); adminRefreshBattles(); } catch(e) { notify(e.message,'error'); }
};
